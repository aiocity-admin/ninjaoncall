<?php
  //date_default_timezone_set('Asia/Kolkata');
	...
	$lang_code 	= '';
	// general fucntions 
	if($type != "generalConfigData" && $type != "signIn" && $type != "isUserExist" && $type != "signup" && $type != "LoginWithFB" && $type != "sendVerificationSMS" && $type != "countryList" && $type != "changelanguagelabel" && $type != "requestResetPassword" && $type != "UpdateLanguageLabelsValue" && $type != "staticPage" && $type != "sendContactQuery" && $type!="UpdateCompany") {
	...
	function getPassengerDetailInfo($passengerID,$cityName){
      ...
      $row[0]['RIDE_BANNER_IMG_NAME']= ($row[0]['RIDE_BANNER_IMG_NAME'] != "")?$tconfig['tsite_upload_images_vehicle_category']."/".$row[0]['RIDE_BANNER_IMG_NAME']:"";
      $row[0]['DELIVERY_GRID_ICON_NAME']= ($row[0]['DELIVERY_GRID_ICON_NAME'] != "")?$tconfig['tsite_upload_images_vehicle_category']."/".$row[0]['DELIVERY_GRID_ICON_NAME']:"";
      $row[0]['DELIVERY_BANNER_IMG_NAME']= ($row[0]['DELIVERY_BANNER_IMG_NAME'] != "")?$tconfig['tsite_upload_images_vehicle_category']."/".$row[0]['DELIVERY_BANNER_IMG_NAME']:"";
			/* fetch value */
			return $row[0];
...
$sql = "SELECT rd.*,cmp.eStatus as cmpEStatus,(SELECT dv.vLicencePlate From driver_vehicle as dv WHERE rd.iDriverVehicleId != '' AND rd.iDriverVehicleId !='0' AND dv.iDriverVehicleId = rd.iDriverVehicleId) as vLicencePlateNo,cmp.vCompany FROM `register_driver` as rd left join `company` as cmp on (cmp.iCompanyId=rd.iCompanyId or rd.iCompanyId=0 ) WHERE rd.iDriverId='$driverId'";
		...
		if(count($Data) > 0)
		{
			...
			if($Data[0]['vImage']!="" && $Data[0]['vImage']!="NONE"){
				...
				$service_sql="SELECT * FROM `driver_registered_service` where iDriverId='$driverId' and (iVehicleCategoryId='Delivery' or iVehicleCategoryId='Ride') ";

    	$data_service = $obj->MySQLSelect($service_sql);
	
	if (count($data_service) > 0)
     {
$Data[0]['SHOW_SELECT']="YES";
     }else
     {
     	$Data[0]['SHOW_SELECT']="NO";

     }


		$service_sql="SELECT * FROM `driver_registered_service` where iDriverId='$driverId' and iVehicleCategoryId='Delivery'";
...
$Data[0]['Delivery']="YES";
     }else
     {
		...
		$service_sql="SELECT * FROM `driver_registered_service` where iDriverId='$driverId' and iVehicleCategoryId='Ride'";
...
$Data[0]['Ride']="YES";
     ...
     else
     {
     	...
     	$service_sql="SELECT * FROM `driver_registered_service` where iDriverId='$driverId' and iVehicleCategoryId!='Delivery' and iVehicleCategoryId!='Ride' ";

    	$data_service = $obj->MySQLSelect($service_sql);
	
	if (count($data_service) > 0)
     {
$Data[0]['Other_Service']="YES";
     }else
     {
     	$Data[0]['Other_Service']="NO";
			...
			if($Data[0]['iDriverVehicleId'] != '' && $Data[0]['iDriverVehicleId'] != '0'){
				...
				$Data[0]['showSelectVehicle'] = '1';

			}
			// }else
			// {
		 //       $obj->sql_query("update driver_vehicle set iDriverId=0 where iDriverId='".$driverId."' and eType!='UberX'");

			// 	$Data[0]['iDriverVehicleId'] != '0';
			// 	$Data[0]['vLicencePlateNo'] = '';
			// 	$Data[0]['vMake'] = '';
			// 	$Data[0]['vModel'] = '';
			// 	$Data[0]['showSelectVehicle'] = '0';
	...
	if($type=="signup"){
		...
		$vImageURL = isset($_REQUEST["vImageURL"]) ? $_REQUEST["vImageURL"] : '';
			//codeEdit for suffix and middle name
		...
		if($user_type == "Passenger"){
			...
			$Data_passenger['iCompanyId'] = $iCompanyId;
			$vImage = 'vImage';
			$iMemberId = 'iDriverId';
		}   
		...
		if(count($check_passenger)>0){
			...
			//codeEdit for suffix and middle name
				$Data_passenger['Suffix']=$Suffix;
					$Data_passenger['MiddleName']=$MiddleName;
			if(SITE_TYPE=='Demo')
			{
  ...
  $Service=isset($_REQUEST["ServiceName"]) ? $_REQUEST['ServiceName'] : '';
          ...
          if($Service!="")
          {
		      ...
		      foreach ($Service as $key => $value)
		       {
		      	...
		      	$query_service="INSERT INTO `driver_registered_service`( `iDriverId`, `iVehicleCategoryId`) VALUES ('$id','".trim($value)."')";
			...
			if(strtolower($user_type)=='driver' && SITE_TYPE=='Live'){
    			if($APP_TYPE == 'UberX' || $APP_TYPE == 'Ride-Delivery-UberX'){
					...
					$Drive_vehicle['iCompanyId'] = $iCompanyId;//"1";
			...
			if(strtolower($user_type)=='driver' && SITE_TYPE=='Demo'){
				...
				if($APP_TYPE == 'Ride-Delivery-UberX' || $APP_TYPE == 'UberX'){
					...
					if($APP_TYPE == 'Ride-Delivery-UberX'){
    					...
    					$Drive_vehicle_ride['iCompanyId'] = $iCompanyId;//"1";
...
$nortification_query="SELECT `Event`, `ActionBy`, `NotifyCompany`, `NotifyProvider`, `NotifyCustomer`, `NotifyAdministrator`, `AdditionalEmail` FROM `nortification_settings` WHERE `Event`='$user_type Signup' and  `ActionBy`='$user_type'";
...
$diff_query="INSERT INTO `performance`(`eType`, `TimeFrom`, `TimeTo`, `TotalTime`,`UserType`) VALUES ('SIGNIN','$date1','$date2','$diff','$UserType')";
...
$obj->sql_query($diff_query);
	...
	if($type=="UpdateCompany")
	{
        ...
        $query="update register_driver set iCompanyId='$iCompanyId' where iDriverId='$iDriverId'";
        $obj->sql_query($query);
        $returnArr['Action'] = "1";
        $returnArr['message'] ="LBL_COMPANY_UPDATED";
        $returnArr['data']=getDriverDetailInfo($iDriverId,'');
        echo json_encode($returnArr);
...
$user_type=trim(str_replace("}", "", $user_type));
		if($fbid == ""){
			$returnArr['Action'] ="0";
			$returnArr['message'] ="LBL_TRY_AGAIN_LATER_TXT";
			echo json_encode($returnArr);exit;
	...
	if ($type == "loadAvailableCab") {
		...
		$DataArr = getOnlineDriverArr($passengerLat,$passengerLon,$address_data,"No","No",$Check_Driver_UFX,$Check_Date_Time,"","",$eType,$ServiceType,$quantity);
	...
	if($type=="getDriverStates"){
		...
		//codeEdited for upload document only for registered services
		/*$sql1= "select * from (SELECT dm.doc_masterid masterid, dm.doc_usertype , dm.doc_name ,dm.ex_status,dm.status,dm.eType, COALESCE(dl.doc_id,  '' ) as doc_id,COALESCE(dl.doc_masterid, '') as masterid_list ,COALESCE(dl.ex_date, '') as ex_date,COALESCE(dl.doc_file, '') as doc_file, COALESCE(dl.status, '') as docstatus FROM document_master dm left  join (SELECT * FROM `document_list` where doc_userid='".$driverId."' ) dl on dl.doc_masterid=dm.doc_masterid   
		where dm.doc_usertype='driver'  and (dm.country='".$vCountry."' OR dm.country='All') and dm.status='Active' $ssql ) t join driver_registered_service cat on t.eType=cat.iVehicleCategoryId and iDriverId='".$driverId."' group by t.masterid";*/
	
$sql1= "select * from (SELECT dm.doc_masterid masterid, dm.doc_usertype , dm.doc_name ,dm.ex_status,dm.status,dm.eType, COALESCE(dl.doc_id,  '' ) as doc_id,COALESCE(dl.doc_masterid, '') as masterid_list ,COALESCE(dl.ex_date, '') as ex_date,COALESCE(dl.doc_file, '') as doc_file, COALESCE(dl.status, '') as docstatus FROM document_master dm left  join (SELECT * FROM `document_list` where doc_userid='".$driverId."' ) dl on dl.doc_masterid=dm.doc_masterid   
		where dm.doc_usertype='driver'  and (dm.country='".$vCountry."' OR dm.country='All') and dm.status='Active') t join driver_registered_service cat on t.eType=cat.iVehicleCategoryId and iDriverId='".$driverId."' group by t.masterid";
...
$sql_doc="SELECT dm.doc_masterid masterid FROM document_master dm   join (SELECT * FROM `document_list` where doc_userid='".$driverId."' ) dl on dl.doc_masterid=dm.doc_masterid   	where dm.doc_usertype='driver'  and (dm.country='".$vCountry."' OR dm.country='All') and dm.status='Active' $ssql";
...
*/
		...
		if($APP_TYPE != 'UberX'){
			...
			if(count($db_drv_vehicle) == 0){
				...
				{
//codeEdited start for manageservices mark as complete
                $sql="SELECT * FROM `driver_registered_service` where iDriverId='$driverId' and (`iVehicleCategoryId` like '%Ride%' or `iVehicleCategoryId` = ' Delivery' or `iVehicleCategoryId`=' Deliver' or `iVehicleCategoryId` = 'Delivery')";
				$db_registered_services= $obj->MySQLSelect($sql);

if(count($db_registered_services)==0)
{
				//$driverVehicleUpload = 'Yes';

	$sql= "SELECT iDriverVehicleId,vCarType FROM `driver_vehicle` WHERE iDriverId='$driverId' and eType='UberX' AND eStatus != 'Deleted'";
				$db_selected_vehicle = $obj->MySQLSelect($sql);
				if(count($db_selected_vehicle) > 0){
					if($db_selected_vehicle[0]['vCarType']=="")
									$driverVehicleUpload = 'No';
								else 
									$driverVehicleUpload = 'Yes';
...
if(count($db_document_registered)<=count($All_uploaded_document))
{
	...
	if ($type == "updateUserProfileDetail") {
		...
		$MiddleName        = isset($_REQUEST["MiddleName"]) ? stripslashes($_REQUEST["MiddleName"]) : '';
		$Suffix        = isset($_REQUEST["Suffix"]) ? stripslashes($_REQUEST["Suffix"]) : '';
    ...
    if($vInviteCode != ""){
       ...
       if($check_inviteCode == "" || $check_inviteCode == "0" || $check_inviteCode == 0){
        ...
        if($iCompanyId!='')
         {
			$old_iCompanyId =  get_value('register_driver', 'iCompanyId', 'vEmail',$vEmail,'','true');
if($old_iCompanyId!=$iCompanyId)
{
$q="update driver_vehicle set vCarType='' where eType='UberX' and iDriverId='$iMemberId'";
$obj->MySQLSelect($q);
}
        $Data_update_User['iCompanyId']=$iCompanyId;
  ...
  $Service=isset($_REQUEST["ServiceName"]) ? $_REQUEST['ServiceName'] : '';
		      ...
		      foreach ($Service as $key => $value)
		       {
		      	...
		      	$query_service="INSERT INTO `driver_registered_service`( `iDriverId`, `iVehicleCategoryId`) VALUES ('$iMemberId','".trim($value)."')";
      ...
      $UservInviteCode = $passengerData[0]['vInviteCode'];
		...
		$Data_update_User['MiddleName']=$MiddleName;
		$Data_update_User['Suffix']=$Suffix;
	...
	if($type == "uploadImageBeforeAndAfterService"){
	...
	if($type == "uploadImage"){
		...
		$uploadtype= isset($_REQUEST['uploadtype'])?$_REQUEST['uploadtype']:'';
		if(trim($uploadtype) =='uploadImageBeforeAndAfterService')
		{
				global $generalobj,$tconfig;

		//$iMemberId 	= isset($_REQUEST['iDriverId'])?clean($_REQUEST['iDriverId']):'';
	...
	$vFile = $generalobj->fileupload($Photo_Gallery_folder,$image_object,$image_name,$prefix='', $vaildExt="pdf,doc,docx,jpg,jpeg,gif,png");	$vImageName = $vFile[0];		
		if($vImageName != ''){				
					//$vImageName=explode("/", $vImageName);
						$returnArr['Action']="1";
						$returnArr['vImageName']= $vImageName;//$vImageName[count($vImageName)-1];			
			}
			else{
			$returnArr['Action']="0";
			$returnArr['message'] ="LBL_TRY_AGAIN_LATER_TXT";
	...
	if($type=='sendContactQuery'){
		...
		if($UserType=='Passenger'){
			$sql="SELECT vName,vLastName,MiddleName,Suffix,vPhone,vEmail FROM register_user WHERE iUserId=$UserId";
			
			$result_data = $obj->MySQLSelect($sql);
			
			}else if($UserType=='Driver'){
			$sql="SELECT vName,vLastName,MiddleName,Suffix,vPhone,vEmail FROM register_driver WHERE iDriverId=$UserId";
...
if ($type == "sendRequestToDrivers") {
    ...
    if($APP_TYPE == "Ride-Delivery-UberX"){  
      ...
      if(count($TripData) > 0){
		...
		$DataArr = getOnlineDriverArr($PickUpLatitude,$PickUpLongitude,$address_data,"Yes","No","No","",$DestLatitude,$DestLongitude,$eType);
		$Data = $DataArr['DriverList'];
		...
		$sqlp = "SELECT iGcmRegId,vName,vLastName,MiddleName,Suffix,vImgName,vFbId,vAvgRating,vPhone,vPhoneCode FROM register_user WHERE iUserId = '".$passengerId."'";
		$passengerData = $obj->MySQLSelect($sqlp);
    ...
    if($eConfirmByUser == "" || $eConfirmByUser == NULL){
      $eConfirmByUser = "No";
       ...
       if($vTripStatus == "Canceled" || $vTripStatus == "Finished" || $vTripStatus == "On Going Trip"){
    ...
    if($APP_TYPE == "Delivery"){
    	...
    	$service_sql="SELECT * FROM `driver_registered_service` where iDriverId='$iDriverId' and (iVehicleCategoryId='Delivery' or iVehicleCategoryId='Ride') ";
    ...
    $sql = "SELECT register_driver.iDriverVehicleId as DriverSelectedVehicleId,make.vMake, model.vTitle, dv.* FROM `driver_vehicle` dv, make, model,register_driver WHERE dv.iDriverId='$iDriverId' AND register_driver.iDriverId = '$iDriverId' AND dv.`iMakeId` = make.`iMakeId` AND dv.`iModelId` = model.`iModelId` AND dv.`eStatus`='Active'".$ssql;
		...
		if (count($Data_Car) > 0)
		{
			...
			$sql = "SELECT count(iDriverVehicleId) as TotalVehicles from driver_vehicle WHERE iDriverId = '".$iDriverId."'";
			$db_Total_vehicle = $obj->MySQLSelect($sql);
			$TotalVehicles = $db_Total_vehicle[0]['TotalVehicles'];
			
			if(count($Data_Car) == 1 && $Data_Car[0]['eType'] == "UberX" && $TotalVehicles == 1 && $APP_TYPE = "Ride-Delivery-UberX"){
				$returnArr['Action']="0";
				$returnArr['message']="LBL_PROVIDER_OTHER_SERVICE_ENABLE_TXT";
				echo json_encode($returnArr);
				exit; 
				}else{
        		$status= "CARS_NOT_ACTIVE";
				...
				if(count($db_vehicle_new) == 0){
					...
					if($TotalVehicles > 0){
						...
						$query="select * from configurations where vName='RIDE_GRID_ICON_NAME'";
$db_icon = $obj->MySQLSelect($query);
$returnArr['icon']='/webimages/icons/VehicleCategory/'.$db_icon[0]['vValue'];
					echo json_encode($returnArr);exit;
				}
...
$returnArr['icon']='/webimages/icons/VehicleCategory/'.$db_icon[0]['vValue'];
	...
	if($type == "GenerateTrip"){
		...
		if ($eStatus == "Requesting" || (($eStatus == "Assign" || $eStatus == "Accepted") && $iCabBookingId != "" && $iCabRequestId == "")) {   
			...
			$sql = "SELECT iDriverVehicleId,vCurrencyDriver,iAppVersion,vName,vLastName,MiddleName,Suffix FROM `register_driver` WHERE iDriverId = '$driver_id'";
			$Data_vehicle = $obj->MySQLSelect($sql);
			...
			*/	
			...
			if($eType == "UberX"){
				...
				$imageuploaddata = get_value('vehicle_category', 'eBeforeUpload, eAfterUpload,NumberOfBeforeUpload,NumberOfAfterUpload', 'iVehicleCategoryId', $iVehicleCategoryId);
				$Data_trips['eBeforeUpload']=$imageuploaddata[0]['eBeforeUpload'];
				$Data_trips['eAfterUpload']=$imageuploaddata[0]['eAfterUpload']; 

			   $Data_trips['iVehicleCategoryId']=$iVehicleCategoryId; 


				$Data_trips['NumberOfBeforeUpload']=$imageuploaddata[0]['NumberOfBeforeUpload']; 
				$Data_trips['NumberOfAfterUpload']=$imageuploaddata[0]['NumberOfAfterUpload']; 
			...
			if($setCron == 'Yes'){
				$passengerDetail = get_value('register_user', 'vName,vLastName,MiddleName,Suffix,vPhone,vPhoneCode', 'iUserId', $passenger_id);
			...
			if ($iTripId > 0) {
				...
				if($iCabBookingId !=""){
					$passengerData = get_value('register_user', 'vName,vLastName,MiddleName,Suffix,vImgName,vFbId,vAvgRating,vPhone,vPhoneCode,iAppVersion', 'iUserId', $passenger_id);
	...
	if($type == "DriverArrived"){
		...
		if($iDriverId != '') {
			...
			if($id>0){
				...
				$sql = "SELECT CONCAT(rd.vName,' ',rd.vLastName) AS driverName, tr.vRideNo, tr.tEndLat,tr.tEndLong,tr.tDaddress,tr.iUserId,tr.eType,rd.iTripId,tr.eTollSkipped,tr.eBeforeUpload,tr.eAfterUpload,tr.vBeforeImageComments,tr.vAfterImageComments,tr.iDriverVehicleId,tr.iVehicleCategoryId,tr.NumberOfBeforeUpload,tr.NumberOfAfterUpload FROM trips as tr,register_driver as rd WHERE tr.iTripId=rd.iTripId AND rd.iDriverId = '".$iDriverId."'";
				...
				$data['vBeforeImageComments']=$result[0]['vBeforeImageComments'];
				$data['vAfterImageComments']=$result[0]['vAfterImageComments'];
				$data['NumberOfBeforeUpload']=$result[0]['NumberOfBeforeUpload'];
				$data['NumberOfAfterUpload']=$result[0]['NumberOfAfterUpload'];

								$returnArr['message'] = $data;


		$sql_img = "SELECT Type,Label  from images_labels where   Sub_Category_Id='".$result[0]['iVehicleCategoryId']."' and Type='Before'  order by Type";

				$result_img_b = $obj->MySQLSelect($sql_img);

				$returnArr['img_Before'] = $result_img_b;


		$sql_img = "SELECT Type,Label  from images_labels where   Sub_Category_Id='".$result[0]['iVehicleCategoryId']."' and Type='After'  order by Type";

				$result_img_a = $obj->MySQLSelect($sql_img);


				$returnArr['img_After'] = $result_img_a;
	...
	if($type == "ProcessEndTrip"){
		...
		$beforeComment = isset($_REQUEST["beforeComment"]) ? $_REQUEST["beforeComment"] : '';
		$afterComment = isset($_REQUEST["afterComment"]) ? $_REQUEST["afterComment"] : '';
		$beforeImagePath = isset($_REQUEST["beforeImagePath"]) ? $_REQUEST["beforeImagePath"] : '';
		$afterImagePath = isset($_REQUEST["afterImagePath"]) ? $_REQUEST["afterImagePath"] : '';

		//tr.eBeforeUpload,tr.eAfterUpload,tr.vBeforeImageComments,tr.vAfterImageComments
		...
		$Data_update_trips['ServiceFeeType']=$Fare_data['ServiceFeeType']; 

		$Data_update_trips['vBeforeImage']=$beforeImagePath; 
		$Data_update_trips['vAfterImage']=$afterImagePath; 
		$Data_update_trips['vBeforeImageComments']=$beforeComment; 
		$Data_update_trips['vAfterImageComments']=$afterComment; 



				//$Data_update_trips['Tokens']=$Fare_data['Tokens']; 

				$Data_update_trips['FixedCommision']=$Fare_data['FixedCommision']; 
				$Data_update_trips['RevenueShare']=$Fare_data['RevenueShare']; 
	...
	if($type=="CollectPayment"){
		...
		$sql = "SELECT vTripPaymentMode,iUserId,iDriverId,iFare,vRideNo,fWalletDebit,fTripGenerateFare,fDiscount,fCommision,fTollPrice,eHailTrip,fOutStandingAmount,ServiceFeeType,RevenueShare,FixedCommision FROM trips WHERE iTripId='$iTripId'";
...
//codeEdited for TotalEarning
    ...
    if($APP_TYPE == "Ride-Delivery-UberX"){  
      ...
      if(count($TripData) > 0){
		...
		$fFlatTripPrice = $data_flattrip['Flatfare'];
		## Checking For Flat Trip ##
		$data_surgePrice = checkSurgePrice($iVehicleTypeId,$scheduleDate);
		...
		if($UserType == "Driver"){
			...
			if($dataType == "PENDING"){
				...
				$sql_all  = "SELECT COUNT(iCabBookingId) As TotalIds FROM cab_booking WHERE iDriverId != '' AND ( eStatus = 'Accepted' || eStatus = 'Assign' ) AND iDriverId='".$iDriverId."'". $ssql1;
								...
								//code edited for show cancel rides
	...
	if($type=="getServiceCategoryTypes"){
		...
		if($eCheck == "Yes"){
			...
			if($userId != "") {
    			...
    			$sql2 = "SELECT vc.iVehicleCategoryId, vc.iParentId,vc.vCategory_".$lang." as vCategory, vc.ePriceType, vt.iVehicleTypeId, vt.vVehicleType_".$lang." as vVehicleType, vt.eFareType, vt.fFixedFare, vt.fPricePerHour, vt.fPricePerKM, vt.fPricePerMin, vt.iBaseFare,vt.fCommision, vt.iMinFare,vt.iPersonSize, vt.vLogo as vVehicleTypeImage, vt.eType, vt.eIconType, vt.eAllowQty, vt.iMaxQty, vt.iVehicleTypeId, fFixedFare,vt.CalculateFare,vt.is_payment_type,vt.payment_type FROM vehicle_category as vc LEFT JOIN vehicle_type AS vt ON vt.iVehicleCategoryId = vc.iVehicleCategoryId WHERE vc.eStatus='Active' AND vt.iVehicleCategoryId='$iVehicleCategoryId' AND vt.iLocationid IN ($GetVehicleIdfromGeoLocation) ORDER BY vt.iDisplayOrder ASC";
    			//AND vt.eType='UberX'
    			$Data = $obj->MySQLSelect($sql2);
    			if(!empty($Data)){
    				for($i=0;$i<count($Data);$i++){
    					if($Data[$i]['CalculateFare']==0 || trim($Data[$i]['CalculateFare'])=="0")
    					{
//codeEdited for remove fare based on Calculate Fare for other services
    					
    					$Data[$i]['fFixedFare_value']="";
    					
						$Data[$i]['fFixedFare']="";;
						$Data[$i]['fPricePerHour_value']="";
    					$Data[$i]['fPricePerHour']= "";
						$Data[$i]['fPricePerKM']= "";
    					$Data[$i]['fPricePerMin']="";
    					$Data[$i]['iBaseFare']="";
    					$Data[$i]['fCommision']="";
...
//codeEdited for issue in update services
if ($action=='Edit' && $eType == "UberX") {	
	# code...
...
/*
  $Service_all=$vCarType;
  

          if(trim($Service_all)!="")
          {
          	$Service_all=explode(',', $Service_all);
	...
	if(trim($value)!='Ride'&& trim($value)!="Delivery")
	{
...
if (count($service_a)==0) {
	...
	if ($type == "displayDocList") {
		...
		$sql_extraFields= "SELECT ef.* FROM document_master dm left join (SELECT * FROM `document_list` where doc_userid='" . $doc_userid . "' ) dl on dl.doc_masterid=dm.doc_masterid  left join ExtraFields ef on ef.doc_masterid=dm.doc_masterid   and ef.vFor='Document' where dm.doc_usertype='".$doc_usertype."' and ef.DocumentTypeId='0' AND (dm.country='".$vCountry."' OR dm.country='All') and dm.status='Active' $ssql order by ef.ID ASC";
		$db_vehicle_extraFields = $obj->MySQLSelect($sql_extraFields);
    ...
    for ($i=1; $i <=5 ; $i++) { 
       $selectExtraField.=",dl.ExtraField_".$i;
		...
		$sql1= "SELECT dm.doc_masterid masterid, dm.doc_usertype , dm.eType,dm.doc_name_EN as doc_name ,dm.ex_status,dm.status, COALESCE(dl.doc_id,  '' ) as doc_id,COALESCE(dl.doc_masterid, '') as masterid_list ,COALESCE(dl.ex_date, '') as ex_date,COALESCE(dl.doc_file, '') as doc_file, COALESCE(dl.status, '') as status ,dl.ExtraField_1,dl.ExtraField_2,dl.ExtraField_3,dl.ExtraField_4,dl.ExtraField_5,dl.DocumentTypeName,dm.IsType as dropdown FROM document_master dm left join (SELECT * FROM `document_list` where doc_userid='" . $doc_userid . "' ) dl on dl.doc_masterid=dm.doc_masterid where dm.doc_usertype='".$doc_usertype."' AND (dm.country='".$vCountry."' OR dm.country='All') and dm.status='Active' $ssql ";
		//codeEdit for doclist with extra fields
		//$sql1= "SELECT dm.doc_masterid masterid, dm.doc_usertype , dm.eType,dm.doc_name_".$vLang." as doc_name ,dm.ex_status,dm.status, COALESCE(dl.doc_id,  '' ) as doc_id,COALESCE(dl.doc_masterid, '') as masterid_list ,COALESCE(dl.ex_date, '') as ex_date,COALESCE(dl.doc_file, '') as doc_file, COALESCE(dl.status, '') as status,ef.* FROM document_master dm left join (SELECT * FROM `document_list` where doc_userid='" . $doc_userid . "' ) dl on dl.doc_masterid=dm.doc_masterid  left join ExtraFields ef on ef.doc_masterid=dm.doc_masterid   and ef.vFor='Document' where dm.doc_usertype='".$doc_usertype."' AND (dm.country='".$vCountry."' OR dm.country='All') and dm.status='Active' $ssql "; already commented
...
//codeedited for extra fields

	//for ($i=0; $i <count($db_vehicle) ; $i++) //{ 	

// $dropdown_q="SELECT ef.* FROM document_master dm right join (SELECT * FROM `document_list` where doc_userid='" . $doc_userid . "' ) dl on dl.doc_masterid=dm.doc_masterid  right join document_type_name ef on ef.doc_masterid=dm.doc_masterid   and ef.vFor='Document' where dm.doc_usertype='".$doc_usertype."'  AND (dm.country='".$vCountry."' OR dm.country='All') and dm.status='Active' $ssql ";
  $dropdown_q="SELECT ef.* FROM document_master dm right join document_type_name ef on ef.doc_masterid=dm.doc_masterid   and ef.vFor='Document' where dm.doc_usertype='".$doc_usertype."'  AND (dm.country='".$vCountry."' OR dm.country='All') and dm.status='Active' $ssql order by ef.ID ASC";


//$dropdown_q="SELECT * FROM `` where `doc_masterid`='".$db_vehicle[$i]['masterid']."' and `vFor`='Document'";
    $dropdown=$obj->MySQLSelect($dropdown_q);
    $returnArr['dropDown']=$dropdown;
/*
    if(count($dropdown)>0)
{*/

	//$extraField_q="SELECT `ID`, `doc_masterid`, `vLabel`, `vFor`, `IsRequired`, `DocumentTypeId` FROM `ExtraFields` where doc_masterid='".$db_vehicle[$i]['masterid']."' and vFor='Document' and DocumentTypeId!='0'";

//$extraField_q="SELECT ef.* FROM document_master dm right join (SELECT * FROM `document_list` where doc_userid='" . $doc_userid . "' ) dl on dl.doc_masterid=dm.doc_masterid  right join ExtraFields ef on ef.doc_masterid=dm.doc_masterid   and ef.vFor='Document' where dm.doc_usertype='".$doc_usertype."' and ef.DocumentTypeId!='0' AND (dm.country='".$vCountry."' OR dm.country='All') and dm.status='Active' $ssql ";
$extraField_q="SELECT ef.* FROM document_master dm right join ExtraFields ef on ef.doc_masterid=dm.doc_masterid   and ef.vFor='Document' where dm.doc_usertype='".$doc_usertype."' and ef.DocumentTypeId!='0' AND (dm.country='".$vCountry."' OR dm.country='All') and dm.status='Active' $ssql order by ef.ID ASC";


    $dropdownFields=$obj->MySQLSelect($extraField_q);
    // if(count($dropdownFields)>0)
    $returnArr['dropdownFields']=$dropdownFields;
//}

//}
//if(count($db_vehicle_extraFields)>0)
$returnArr['ExtraField']=$db_vehicle_extraFields;
   ...
   if($APP_TYPE == "Delivery"){
	...
	if ($type == "uploaddrivedocument") {
		...
		$DocumentTypeName = isset($_REQUEST['DocumentTypeName']) ? clean($_REQUEST['DocumentTypeName']) : '';

//codeEdit for save extra fields
			$Data_Update= array();

//$query="select * from ExtraFields where doc_masterid='$doc_masterid' and vFor='Document' and DocumentTypeId='0'";
   // $db_extraFields = $obj->MySQLSelect($query);
    for ($z=1; $z <=5 ; $z++) { 

      		$extra = isset($_REQUEST["ExtraField_".$z]) ? clean($_REQUEST["ExtraField_".$z]) : '';

       			$Data_Update["ExtraField_".$z] = $extra;

    }

$Data_Update["DocumentTypeName"]=$DocumentTypeName;
		if($doc_usertype == "vehicle"){
			$doc_usertype = "car";
		}
		$doc_userid = ($doc_usertype == 'car') ? $iDriverVehicleId : $iMemberId;
	...
	if($type == "displayWayBill"){
		$driverId = isset($_REQUEST['iDriverId'])?clean($_REQUEST['iDriverId']):'';
		$userType = isset($_REQUEST['UserType'])?clean($_REQUEST['UserType']):'Driver';
		$driver_detail = get_value('register_driver', 'vName,vLastName,MiddleName,Suffix,vCurrencyDriver,vLang', 'iDriverId', $driverId);
		if($APP_TYPE == "Ride-Delivery-UberX"){
		...
		$tripData = $obj->MySQLSelect($sql);
		if(count($tripData) > 0){
			$passenger_detail = get_value('register_user', 'vName,vLastName,MiddleName,Suffix,eHail', 'iUserId', $tripData[0]['iUserId']);
	...
	if($type=="StartHailTrip"){
		...
		$Data_update_driver['vAvailability']="Not Available";
		$id = $obj->MySQLQueryPerform("register_driver",$Data_update_driver,'update',$where);
		$sql = "SELECT iDriverVehicleId,vCurrencyDriver,iAppVersion,vName,vLastName,MiddleName,Suffix FROM `register_driver` WHERE iDriverId = '$driverId'";
		$Data_vehicle = $obj->MySQLSelect($sql);
		$message_arr = array();
		$message_arr['iDriverId'] = $driverId;
		$message_arr['Message'] = $DriverMessage;
...
//codeEdited for provider can only provide only those services that has been assigned by company
			$iCompanyId= get_value('register_driver', 'iCompanyId', 'iDriverId',$iDriverId,'','true');	
...
$ser_cat = $obj->MySQLSelect($sql_cat);


				$returnArr['ServiceCount'] = count($ser_cat);			

		$sql = "SELECT vc.iVehicleCategoryId, vc.vCategory_".$languageCode." as '".$returnName."' FROM vehicle_category as vc join driver_registered_service drs on vc.vCategory_EN=drs.iVehicleCategoryId  WHERE drs.iDriverId=".$iDriverId." and vc.eStatus='Active' AND vc.iParentId='".$iVehicleCategoryId."'".$ssql_category ." GROUP BY vc.iVehicleCategoryId ". $limit."";
		...
		if(count($vehicleCategoryDetail) >0)
		{
			...
			$vehicleCategoryData=array();//codeEdited
			if($iVehicleCategoryId != "" && ($iVehicleCategoryId == 0 || $iVehicleCategoryId == "0")){
				$i=0;
				$z=0;//codeEdited
...
//codeEdited
	$sql = "SELECT distinct vcc.vCategory_" .$languageCode. " as vTitle,vcc.iVehicleCategoryId FROM `vehicle_category`  as vcc join vehicle_type as vt on vcc.iVehicleCategoryId=vt.iVehicleCategoryId join company_services as cs on cs.ServiceId=vt.iVehicleTypeId WHERE  cs.CompanyId='$iCompanyId' and  vcc.iParentId='".$iVehicleCategoryId."' AND vcc.eStatus='Active'";
...
//$vehicleCategoryData[$i]['SubCategory'] = $subCategoryData;
				...
				{
					$vehicleCategoryData[$z]= $vehicleCategoryDetail[$i];
					$vehicleCategoryData[$z]['SubCategory'] = $subCategoryData;$z++;
			}
///////
...
if($z==0)
{
	$sql = "SELECT * FROM `language_label` WHERE vLabel like '%LBL_SERVICE_NOT_ENABLED_FOR_COMPANY%' AND vCode = '".$languageCode."'";
	$db_label = $obj->MySQLSelect($sql);

$vehicleCategoryData[0]=array("vCategory"=>$db_label[0]['vValue']);
$vehicleCategoryData[0]['SubCategory']=array();
}
	...
	//codeEdited for provider can only provide only services that has been assigned to company and provider

	/*
code commented for do as it was
	$iCompanyId= get_value('register_driver', 'iCompanyId', 'iDriverId',$iDriverId,'','true');	

	$vCarType= get_value('driver_vehicle', 'vCarType', 'iDriverId',$iDriverId,'','true');		
$vCarType=explode(',', $vCarType);
$subQuery='(';
for ($i=0; $i <count($vCarType) ; $i++) { 
	$VehicleTypeId=trim($vCarType[$i]);
	$subQuery.="  vt.iVehicleTypeId='$VehicleTypeId' or";
}
	$subQuery=rtrim($subQuery,'or');
	$subQuery.=" ) and ";
		$sql2 = "SELECT iVehicleTypeId, vVehicleType_".$languageCode." as vTitle,eFareType,eAllowQty,iMaxQty,fFixedFare,fPricePerHour,iLocationid from vehicle_type as vt join company_services as cs on vt.iVehicleTypeId=cs.ServiceId  where  $subQuery  cs.CompanyId='$iCompanyId' and iVehicleCategoryId in($iVehicleCategoryId)".$ssql ; 
*/
    ...
    $db_vCarType = $obj->MySQLSelect($sql);					
		...
		if(count($vehicleDetail) >0){		
			...
			$returnArr['Action'] = "0";
			//$returnArr['message'] ="LBL_NO_DATA_AVAIL";		
$returnArr['message'] ="LBL_SERVICE_NOT_ENABLED_FOR_PROVIDER";	
			...
			if(count($checkdata_exists) > 0){
				if(SITE_TYPE=="Demo") {
					if($checkdata_exists[0]['call_limit'] <= 5) {
						...
						// $returndata['MaskedNumber'] = $checkdata_exists[0]['mask_number'];
	...
	if($type == "ChargePassengerOutstandingAmount"){
		...
		$sqld = "SELECT vStripeCusId,vStripeToken,vCurrencyPassenger FROM register_user WHERE iUserId = '".$iMemberId."'";
