...
date1= date1.getFullYear() + '-' +
 ...
 $.ajax({
           ...
           data: {"loadtime":seconds,"beforeload":date1,"afterload":date2,"UserType":"COMPANY","eType":"STATISTICS_ONLINE_PROVIDERS"}, 
...
$script   = "statisticsReportOnlineProvidersCompany";
...
if($list=="Yes")
{
 ...
 $sql="select count(numberofdriver) as Total from ( select CONCAT(rd.vName,' ',rd.MiddleName,' ',rd.vLastName) as numberofdriver, c.vCompany,c.iCompanyId,vt.iVehicleTypeId,vt.eType,vcp.iVehicleCategoryId,vcp.vCategory_EN,rd.tRegistrationDate from company c left join  register_driver rd on c.iCompanyId=rd.iCompanyId right join driver_log_report dlr on (dlr.iDriverId=rd.iDriverId and dlr.dLoginDateTime>'$dateLimitFrom')  left join driver_vehicle dv on rd.iDriverId=dv.iDriverId left join vehicle_type vt on  dv.vCarType like concat('%',vt.iVehicleTypeId,'%') LEFT JOIN vehicle_category AS vc ON vt.iVehicleCategoryId = vc.iVehicleCategoryId left join (SELECT * FROM  vehicle_category  WHERE  `iParentId`=0) as vcp on vcp.iVehicleCategoryId=vc.iParentId  $subQuery  $groupby_inner   ) x   ";

}
else
{

 $sql="select count(numberofdriver) as Total from (select count(numberofdriver) as numberofdriver, vCompany,iCompanyId,iVehicleCategoryId,eType,vCategory_EN from ( select rd.iDriverId as numberofdriver, c.vCompany,c.iCompanyId,vt.iVehicleTypeId,vt.eType,vcp.iVehicleCategoryId,vcp.vCategory_EN from company c left join  register_driver rd on c.iCompanyId=rd.iCompanyId right join driver_log_report dlr on (dlr.iDriverId=rd.iDriverId and dlr.dLoginDateTime>'$dateLimitFrom')  left join driver_vehicle dv on rd.iDriverId=dv.iDriverId left join vehicle_type vt on  dv.vCarType like concat('%',vt.iVehicleTypeId,'%') LEFT JOIN vehicle_category AS vc ON vt.iVehicleCategoryId = vc.iVehicleCategoryId left join (SELECT * FROM  vehicle_category  WHERE  `iParentId`=0) as vcp on vcp.iVehicleCategoryId=vc.iParentId  $subQuery $groupby_inner ) x  $groupby) t";
}
...
if($list=="Yes")
{
 ...
 //$sql_company_wallet="select  numberofdriver, vCompany,iCompanyId,tRegistrationDate,iDriverId,iVehicleCategoryId,eType,vCategory_EN  from ( select CONCAT(rd.vName,' ',rd.MiddleName,' ',rd.vLastName)  as numberofdriver, c.vCompany,c.iCompanyId,vt.iVehicleTypeId,vt.eType,rd.tRegistrationDate,rd.iDriverId,vcp.iVehicleCategoryId,vcp.vCategory_EN   FROM company c left join  register_driver rd on c.iCompanyId=rd.iCompanyId left join driver_vehicle dv on rd.iDriverId=dv.iDriverId left join vehicle_type vt on  dv.vCarType like concat('%',vt.iVehicleTypeId,'%') LEFT JOIN vehicle_category AS vc ON vt.iVehicleCategoryId = vc.iVehicleCategoryId left join (SELECT * FROM  vehicle_category  WHERE  `iParentId`=0) as vcp on vcp.iVehicleCategoryId=vc.iParentId  $subQuery group by rd.iDriverId  ) x    $ord   LIMIT $start, $per_page";
  ...
  $sql_company_wallet=" select CONCAT(rd.vName,' ',rd.MiddleName,' ',rd.vLastName) as numberofdriver, c.vCompany,c.iCompanyId,vt.iVehicleTypeId,vt.eType,vcp.iVehicleCategoryId,vcp.vCategory_EN,rd.tRegistrationDate,rd.iDriverId,dlr.dLoginDateTime,dlr.dLogoutDateTime from company c left join  register_driver rd on c.iCompanyId=rd.iCompanyId right join driver_log_report dlr on (dlr.iDriverId=rd.iDriverId and dlr.dLoginDateTime>'$dateLimitFrom')  left join driver_vehicle dv on rd.iDriverId=dv.iDriverId left join vehicle_type vt on  dv.vCarType like concat('%',vt.iVehicleTypeId,'%') LEFT JOIN vehicle_category AS vc ON vt.iVehicleCategoryId = vc.iVehicleCategoryId left join (SELECT * FROM  vehicle_category  WHERE  `iParentId`=0) as vcp on vcp.iVehicleCategoryId=vc.iParentId  $subQuery  $groupby_inner $ord2  LIMIT $start, $per_page";
...
else
{
 ...
 //$sql_company_wallet="select sum(numberofdriver) as numberofdriver, vCompany,iCompanyId  from (select count(numberofdriver) as numberofdriver, vCompany,iCompanyId  from ( select  rd.iDriverId as numberofdriver, c.vCompany,c.iCompanyId,vt.iVehicleTypeId,vt.eType   FROM company c left join  register_driver rd on c.iCompanyId=rd.iCompanyId left join driver_vehicle dv on rd.iDriverId=dv.iDriverId left join vehicle_type vt on  dv.vCarType like concat('%',vt.iVehicleTypeId,'%') LEFT JOIN vehicle_category AS vc ON vt.iVehicleCategoryId = vc.iVehicleCategoryId left join (SELECT * FROM  vehicle_category  WHERE  `iParentId`=0) as vcp on vcp.iVehicleCategoryId=vc.iParentId  $subQuery group by rd.iDriverId  ) x  group by iCompanyId,iVehicleTypeId,eType ) xx   group by iCompanyId   $ord  LIMIT $start, $per_page";
  ...
  $sql_company_wallet="select count(numberofdriver) as numberofdriver, vCompany,iCompanyId,iVehicleCategoryId,eType,vCategory_EN from ( select rd.iDriverId as numberofdriver, c.vCompany,c.iCompanyId,vt.iVehicleTypeId,vt.eType,vcp.iVehicleCategoryId,vcp.vCategory_EN from company c left join  register_driver rd on c.iCompanyId=rd.iCompanyId right join driver_log_report dlr on (dlr.iDriverId=rd.iDriverId and dlr.dLoginDateTime>'$dateLimitFrom') left join driver_vehicle dv on rd.iDriverId=dv.iDriverId left join vehicle_type vt on  dv.vCarType like concat('%',vt.iVehicleTypeId,'%') LEFT JOIN vehicle_category AS vc ON vt.iVehicleCategoryId = vc.iVehicleCategoryId left join (SELECT * FROM  vehicle_category  WHERE  `iParentId`=0) as vcp on vcp.iVehicleCategoryId=vc.iParentId  $subQuery $groupby_inner ) x  $groupby  $ord  LIMIT $start, $per_page";
}
...
?>
 ...
 <th>  <a href="javascript:void(0);" onClick="Redirect(1,<?php if($sortby == '1'){ echo $order; }else { ?>0<?php } ?>)"> Provider <?php if ($sortby == 1) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
                 ...
                 <? if(trim($iVehicleCategoryId)!="" && trim($iVehicleCategoryId)!="null" ) { ?>
  ...
  <th>  <a href="javascript:void(0);" onClick="Redirect(4,<?php if($sortby == '4'){ echo $order; }else { ?>0<?php } ?>)">Service Category <?php if ($sortby == 4) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a>
                                                  ...
                                                  <? } ?>
                                                         ...
                                                         <th>  <a href="javascript:void(0);" onClick="Redirect(5,<?php if($sortby == '5'){ echo $order; }else { ?>0<?php } ?>)">Online From<?php if ($sortby == 5) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a>
                                                    </th>
...
<!--                                                                                                       <td><?=$value['vCompany'];?></td>
 -->
                                                       <?php if($list!="Yes") { ?>
                                                 <td><a href="javascript:window.open('statisticsReportOnlineProvidersCompany.php?search=1&list=Yes&company=<?=$value['iCompanyId'];?>&keyword=<?=$value['vCompany'];?>&eStatus=Active&iVehicleCategoryId=<?=$VehicleCategory;?>','_blank')"><?=$value['numberofdriver'];?></a> </td>
<? } else {
?>
...
post('statisticsReportOnlineProvidersCompany.php', {search:1,providers:provider,iVehicleCategoryId:iVehicleCategoryId,list:list,sortby:sortby,order:order });
...
var sortby=$("#sortby").val();
var order=$("#order").val();
post('admin/ExportStatisticsReport/exportOnlineProvides.php', {search:1,providers:provider,company:company,iVehicleCategoryId:iVehicleCategoryId,list:list,sortby:sortby,order:order });
...
$("#reset").click(function(){
  ...
  window.location.href="statisticsReportOnlineProvidersCompany.php";
});
