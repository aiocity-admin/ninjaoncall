<?php
    ...
    $FixedCommission=get_value('vehicle_type', 'FixedCommission', 'iVehicleTypeId',$iVehicleTypeId,'','true');
    $ServiceFeeType=get_value('vehicle_type', 'ServiceFeeType', 'iVehicleTypeId',$iVehicleTypeId,'','true');
...
//$Commision_Fare =round((($total_fare_for_commission_ufx * $Fare_data[0]['fCommision']) / 100), 2); 	
           ...
           $ServiceFeeType=get_value('vehicle_type', 'ServiceFeeType', 'iVehicleTypeId',$vehicleTypeID,'','true');
   

			//	$Commision_Fare = round((($total_fare * $Fare_data[$i]['fCommision']) / 100), 2);

//Code edited for Commision
           $Commision_Fare;
if($ServiceFeeType=='1')
{
$Commision_Fare  =round((($total_fare * $Fare_data[$i]['fCommision']) / 100), 2); // codeEdited
}
else if($ServiceFeeType=='2')
{
$Commision_Fare  =$Fare_data[$i]['FixedCommission'];

}
/*else if($ServiceFeeType=='3')
{
$result['fCommision'] =$Fare_data[0]['FixedCommission'];
$Tokens=$Fare_data[0]['FixedCommission'];
}*/
else if($ServiceFeeType=='4')
{
$Commision_Fare =$Fare_data[$i]['FixedCommission']+round((($total_fare * $Fare_data[$i]['fCommision']) / 100), 2); 
}
/*else if($ServiceFeeType=='5')
{
$result['fCommision'] =round((($fTripGenerateFare_For_Commission * $Fare_data[0]['fCommision']) / 100), 2); 
$ReCommission=round((($fTripGenerateFare_For_Commission * $Fare_data[0]['fCommision']) / 100), 2);
$Tokens=$Fare_data[0]['FixedCommission'];


}*/
else if($ServiceFeeType=='6')
{
$Commision_Fare=$Fare_data[$i]['FixedCommission']*$iQty; 
}
	...
	function calculateFare($totalTimeInMinutes_trip, $tripDistance, $vehicleTypeID, $iUserId, $priceRatio, $startDate = "", $endDate = "", $couponCode = "", $tripId, $fMaterialFee =0, $fMiscFee =0, $fDriverDiscount =0, $waitingTime = 0) {
		...
		$defaultCurrency = get_value('currency', 'vName', 'eDefault', 'Yes', '', 'true');
		    ...
		    //$FixedCommission=get_value('vehicle_type', 'FixedCommission', 'iVehicleTypeId',$vehicleTypeID,'','true');
    $ServiceFeeType=get_value('vehicle_type', 'ServiceFeeType', 'iVehicleTypeId',$vehicleTypeID,'','true');
    // $Tokens=0;
     $ReCommission=0;
       $FixedCommission=0;
    $sql="select fPickUpPrice,fNightPrice,iQty,eFareType,eFlatTrip,fFlatTripPrice,fVisitFee,fTollPrice,eTollSkipped,fWaitingFees,eHailTrip from trips where iTripId='".$tripId."'";
...
if($ServiceFeeType=='1')
{
$result['fCommision'] =round((($fTripGenerateFare_For_Commission * $Fare_data[0]['fCommision']) / 100), 2); // codeEdited
$ReCommission =round((($fTripGenerateFare_For_Commission * $Fare_data[0]['fCommision']) / 100), 2);
}
else if($ServiceFeeType=='2')
{
$result['fCommision'] =$Fare_data[0]['FixedCommission'];
$FixedCommission=$Fare_data[0]['FixedCommission'];
...
{
$result['fCommision'] =$Fare_data[0]['FixedCommission']*$iQty; 
$FixedCommission=$Fare_data[0]['FixedCommission']*$iQty;
...
{
$result['fCommision'] =$Fare_data[0]['FixedCommission']+round((($fTripGenerateFare_For_Commission * $Fare_data[0]['fCommision']) / 100), 2); 
$FixedCommission=$Fare_data[0]['FixedCommission'];
$ReCommission= round((($fTripGenerateFare_For_Commission * $Fare_data[0]['fCommision']) / 100), 2);
}
		...
		$finalFareData['fCommision'] =$result['fCommision']; //codeEdit
        ...
        $finalFareData['ServiceFeeType']=$ServiceFeeType;//codeEdit
/*        $finalFareData['Tokens']=$Tokens;//codeEdit
*/
          $finalFareData['FixedCommision']=$FixedCommission;//codeEdit
            $finalFareData['RevenueShare']=$ReCommission;//codeEdit
...
$Commision_Fare =$Fare_data[$i]['FixedCommission']*$iQty; 
  ...
  function checkDistanceWithGoogleDirections($tripDistance, $startLatitude, $startLongitude, $endLatitude, $endLongitude, $isFareEstimate = "0", $vGMapLangCode = "",$isReturnArr=false) {
	...
	*/
	...
	function getOnlineDriverArr($sourceLat, $sourceLon,$address_data=array(),$DropOff="No",$From_Autoassign="No",$Check_Driver_UFX="No",$Check_Date_Time="",$destLat="", $destLon="",$eType = "Ride",$ServiceType="",$quantity=1) {
    ...
    if($Check_Driver_UFX == "No"){
      ...
      $j=0;
			...
			{
//codeedited for show only actived service provider
				$sql_cat="";
if($eType=='Ride')
{
$sql_cat="SELECT iVehicleCategoryId from driver_registered_service where iDriverId='".$Data[$i]['iDriverId']."' and Status='1' and iVehicleCategoryId='Ride'";
}
else   if($eType=='Deliver' || $eType=='Delivery')
{
$sql_cat="SELECT iVehicleCategoryId from driver_registered_service where iDriverId='".$Data[$i]['iDriverId']."' and Status='1' and iVehicleCategoryId='Delivery'";	
}else {
	$sql_cat="SELECT drs.iVehicleCategoryId,vt.eType FROM `vehicle_type` vt join vehicle_category vc on vt.iVehicleCategoryId=vc.iVehicleCategoryId join (SELECT iVehicleCategoryId,vCategory_EN FROM vehicle_category where iParentId='0' ) vcp on vcp.iVehicleCategoryId=vc.iParentId right join driver_registered_service drs on drs.iVehicleCategoryId= vcp.vCategory_EN  and drs.iDriverId='".$Data[$i]['iDriverId']."' and drs.Status='1' where vt.iVehicleTypeId='$ServiceType' GROUP by vcp.vCategory_EN";
}
$sql_cat = $obj->MySQLSelect($sql_cat);

if(count($sql_cat)>0 || $ServiceType=="")
{
...
if($ServiceType!="")
{
//if($ServiceType!=""&&$eType!="Ride"&&$eType!="Delivery"&&trim($eType)!="")
$FixedCommission=get_value('vehicle_type', 'FixedCommission', 'iVehicleTypeId',$ServiceType,'','true');
$fCommision=get_value('vehicle_type', 'fCommision', 'iVehicleTypeId',$ServiceType,'','true');
$ServiceFeeType=get_value('vehicle_type', 'ServiceFeeType', 'iVehicleTypeId',$ServiceType,'','true');
$fFixedFare=get_value('vehicle_type', 'fFixedFare', 'iVehicleTypeId',$ServiceType,'','true');
$eFareType=get_value('vehicle_type', 'eFareType', 'iVehicleTypeId',$ServiceType,'','true');

$Commision_Fare=$WALLET_MIN_BALANCE;
if($ServiceFeeType=='1'&&$eFareType=="Fixed")
{
$Commision_Fare  =round((($fFixedFare * $fCommision) / 100), 2); // codeEdited
}
else if($ServiceFeeType=='2')
{
$Commision_Fare  =$FixedCommission;
}

else if($ServiceFeeType=='4'&&$eFareType=="Fixed")
{
$Commision_Fare =$FixedCommission+round((($fFixedFare * $fCommision) / 100), 2); 
}

else if($ServiceFeeType=='6'&&$eFareType=="Fixed")
{
$Commision_Fare=$FixedCommission*$quantity; 
}
else
{
	$Commision_Fare=$WALLET_MIN_BALANCE;
 ...
 if($Commision_Fare > $user_available_balance){
...
$Data[$i]['ACCEPT_CASH_TRIPS'] = "Yes";
    ...
    if($tripData[0]['eType'] == "UberX" && $tripData[0]['eFareType'] != "Regular"){
      $returnArr['tDaddress'] = "";
		...
		if ($tripData[0]['vBeforeImage'] != "") {
			//$returnArr['vBeforeImage'] = $tconfig['tsite_upload_trip_images'] . $tripData[0]['vBeforeImage']; codeEdit 
			$returnArr['vBeforeImage'] =  $tripData[0]['vBeforeImage'];
			$returnArr['vBeforeImageComments'] =  $tripData[0]['vBeforeImageComments'];			

			$returnArr['vBeforeImage_url']= $tconfig['tsite_upload_trip_images'];
		}
		...
		if ($tripData[0]['vAfterImage'] != "") {
			//$returnArr['vAfterImage'] = $tconfig['tsite_upload_trip_images'] . $tripData[0]['vAfterImage']; codeedit
			$returnArr['vAfterImage'] =  $tripData[0]['vAfterImage'];
			$returnArr['vAfterImageComments'] =  $tripData[0]['vAfterImageComments'];			
			$returnArr['vAfterImage_url']=$tconfig['tsite_upload_trip_images'];
      ...
      $iFare_Detail_Earning = $tripData[0]['fTripGenerateFare'] - $fCommision - $tripData[0]['fTax1'] - $tripData[0]['fTax2'] - $tripData[0]['fOutStandingAmount'];
		...
		$SurgePriceFactor = strval($surgePrice);
		//codeEdited for remove fare details
    ...
    $returnArr['fTax2'] = "";// formatNum($fTax2);
    	//End codeEdited for remove fare details
        ...
        if($PAGE_MODE == "DISPLAY"){
          ...
          if($fOutStandingAmount > 0) {
			...
			if ($tripData[0]['eFareType'] == "Regular") {
				...
				$tripFareDetailsArr[$i + $countUfx][$languageLabelsArr['LBL_BASE_FARE_SMALL_TXT']] = ($iActive != "Canceled")?$currencySymbol . $returnArr['iBaseFare']:"--";
				//code undo for display fee end trip
	//$tripFareDetailsArr[$i + $countUfx][$languageLabelsArr['LBL_BASE_FARE_SMALL_TXT']] = ($iActive != "Canceled")? $tripData[0]['fCommision'] :"--";
...
function UpdateTripOutstandingAmount($iTripId,$ePaidByPassenger = "No",$ePaidToDriver = "No")
{
  ...
  $iVehicleTypeId==$tripdata[0]['vTripPaymentMode'];
