...
if($result_nortification[0]['AdditionalEmail']!="")
{
      ...
      if($result_nortification[0]['NotifyAdministrator']=="on")
			...
			if(count($db_user) > 0){			
				...
				$tRegistrationDate	= Date('Y-m-d H:i:s');
				if($femail != "") {
					$sql = "INSERT INTO register_driver (vFbId,vName, vLastName, vEmail, iCompanyId, eStatus,vImage,eGender,vLang,vCurrencyDriver,vRefCode,dRefDate,tRegistrationDate) VALUES ('".$fbid."','".$fbfirstname."', '".$fblastname."', '".$femail."', '0', 'Inactive','','".$fbgender."','".$lang."','".$curr."','".$refercode."','".$dRefDate."','".$tRegistrationDate."')";
...
/*					if($ALLOW_SERVICE_PROVIDER_AMOUNT == "Yes"){
					...
					if($APP_TYPE == 'Ride-Delivery-UberX') {
						if(SITE_TYPE=='Demo')
						{
							...
							$Drive_vehicle_Ride['iCompanyId'] = "0";
							...
							$Drive_vehicle_Deliver['iCompanyId'] = "0";
