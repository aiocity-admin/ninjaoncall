...
$sub_query="";
		...
		$sub_query=",iCompanyId='$iCompanyId'";
		$old_com=$obj->MySQLSelect("SELECT  `iCompanyId` FROM $tbl $where");
		if(trim($old_com[0]['iCompanyId'])!=trim($iCompanyId))
		{
		$obj->sql_query("update driver_vehicle set vCarType='' $where");
		...
		$query = $q ." `".$tbl."` SET
			...
			`vCountry` = '".$vCountry."',
			`vCode` = '".$vCode."',
			`vPhone` = '".$phone."' $sub_query $str".$where;
		$obj->sql_query($query);
