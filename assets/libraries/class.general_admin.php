	...
	Class General_admin
	{
		...
		function check_member_login()
		{
			...
			//echo basename($_SERVER['PHP_SELF']); die;
				...
				if($sess_iGroupId == '5' && basename($_SERVER['PHP_SELF']) == "dashboard.php") {
				header("Location:".$tconfig["tsite_url_main_admin"]."barangay_providers.php");
        		exit;
			}
			/* else if($sess_iGroupId == '5' && basename($_SERVER['PHP_SELF']) != "driver_action.php" && basename($_SERVER['PHP_SELF']) != "barangay_providers.php" && basename($_SERVER['PHP_SELF']) != "driver_action.php"  && basename($_SERVER['PHP_SELF']) != "driver_document_action.php"  && basename($_SERVER['PHP_SELF']) != "manage_service_type.php" && basename($_SERVER['PHP_SELF']) != "add_availability.php"  && basename($_SERVER['PHP_SELF']) != "ajax_estimate_by_vehicle_type.php" )
			{
	        header("Location:".$tconfig["tsite_url_main_admin"]."barangay_providers.php" );
...
*/
        ...
        $sql = "SELECT SUM(fTripGenerateFare) as totalTripAmount,SUM(fCommision) as totalCommissionAmount,SUM(fOutStandingAmount) as totalOutstandingAmount FROM trips as tr WHERE eDriverPaymentStatus = 'Unsettelled' AND vTripPaymentMode='Card' AND iDriverId = '".$id."'".$ssql;
				$data = $obj->MySQLSelect($sql);
        //$total = ($data[0]['totalAmount'] != "")?$data[0]['totalAmount']:'0.00';
    	...
    	public function getAllOutstandingAmountCountbyDriverId($id,$ssql)
		{
			...
			if($id != "") {
				global $obj;
        $sql = "SELECT SUM(fOutStandingAmount) as totalAmount FROM trips AS tr WHERE vTripPaymentMode='Cash' AND eDriverPaymentStatus = 'Unsettelled' AND iDriverId = '".$id."'".$ssql;
				$data = $obj->MySQLSelect($sql);
				$total = ($data[0]['totalAmount'] != "")?$data[0]['totalAmount']:'0.00';
			}
			return number_format($total,2);
        ...
        //get OutstandingAmount from driver for cash  trips
				$sql = "SELECT SUM(fOutStandingAmount) AS totalAmount FROM trips AS tr WHERE eDriverPaymentStatus = 'Unsettelled' AND vTripPaymentMode='Cash' AND iDriverId = '".$id."'".$ssql;
				$data = $obj->MySQLSelect($sql);
				...
				$sql = "SELECT IFNULL( SUM( IFNULL( fTripGenerateFare, 0 ) ) + SUM( IFNULL( fTipPrice, 0 ) ) , 0 ) - IFNULL( SUM( IFNULL( fCommision, 0 ) ) , 0 ) - IFNULL( SUM( IFNULL( fOutStandingAmount, 0 ) ) , 0 ) AS amounts FROM trips  AS tr WHERE eDriverPaymentStatus = 'Unsettelled' AND vTripPaymentMode='Card' AND iDriverId = '".$id."'".$ssql;
				$data = $obj->MySQLSelect($sql);
				...
				$total = number_format($cardTotal-$cashCommision-$OutstandingAmount+$walletpromocodeTotal,2);
			}
			return $total;
		}
		...
		function go_to_home() {
			...
			// print_r($_SESSION); die;
					$isConferenceAdmin = isset($_SESSION['isConferenceAdmin']) ? $_SESSION['isConferenceAdmin']:'';
if ($isConferenceAdmin!="") 
{
	if ($_SESSION['sess_iGroupId']=="4") {
		# code...

	$url = "conference_notifications.php";
	}
	elseif ($_SESSION['sess_iGroupId']=="5") {
		
			$url = "barangay_providers.php";
	}

}else if($sess_iAdminUserId != "") {
				$url = "dashboard.php";
			}
