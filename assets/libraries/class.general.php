...
//codeEdit for compress img
$source_image=$vfile;
 $compress_image=$ftppath1;
    ...
    function send_email_user($type, $db_rec = '', $newsid = '') {
        ...
        switch ($type) {
             case "DOCUMENT_STATUS_CHANGED":
                ...
                $key_arr = Array("#Document#","#Url#","#Name#","#Status#", "#MailFooter#");
                $val_arr = Array($db_rec['Document'],$db_rec['Url'],$db_rec['Name'],$db_rec['Status'],$MAIL_FOOTER);
                break;
               case "RESUBMIT_DOCUMENT":
                $to_email = $db_rec['EMAIL'];
                $key_arr = Array("#Document#","#Url#","#Name#", "#MailFooter#");
                $val_arr = Array($db_rec['Document'],$db_rec['Url'],$db_rec['Name'],$MAIL_FOOTER);
                break;
                case "SERVICES_UPDATED_BY_COMPANY":
                $to_email = $db_rec['EMAIL'];
                $key_arr = Array("#SERVICES#", "#MESSAGE#", "#MailFooter#");
                $val_arr = Array($db_rec['SERVICES'],$db_rec['MESSAGE']. $MAIL_FOOTER);
                break;
                case "DRIVER_REQUEST_FOR_SERVICES_COMPANY":
                $to_email = $db_rec['COMPANY'];
                $key_arr = Array("#SERVICES#","EMAIL","NAME","#MAILFOOTER#");
                $val_arr = Array($db_rec['SERVICES'],$db_rec['EMAIL'],$db_rec['NAME'],$MAIL_FOOTER);
                break;
            ...
            case "DRIVER_REGISTRATION_ADMIN":
                ...
                break;
                  ...
                  case "DRIVER_REGISTRATION_COMPANY":
                $to_email = $db_rec['COMPANY'];//adding case for company email
                $replyto = $db_rec['EMAIL'];
                $key_arr = Array("#NAME#", "#EMAIL#", "#MAILFOOTER#");
                $val_arr = Array($db_rec['NAME'], $db_rec['EMAIL'], $MAIL_FOOTER);
                break;
                case "DRIVER_REGISTRATION_ADDITIONALEMAIL":
                  
                $to_email = $db_rec['ADDITIONALEMAIL'];//adding case for additional email
                $key_arr = Array("#NAME#", "#EMAIL#", "#MAILFOOTER#");
                $val_arr = Array($db_rec['NAME'], $db_rec['EMAIL'], $MAIL_FOOTER);
                break;

              case "MEMBER_REGISTRATION_ADDITIONALEMAIL":
              ...
              case "MEMBER_REGISTRATION_ADMIN":
                $to_email = $ADMIN_EMAIL; //adding case for admin email
                $replyto = $db_rec['EMAIL'];
                $key_arr = Array("#NAME#", "#EMAIL#", "#MAILFOOTER#");
                $val_arr = Array($db_rec['NAME'], $db_rec['EMAIL'], $MAIL_FOOTER);
                break;

             case "COMPANY_REGISTRATION_ADDITIONALEMAIL":
              ...
              case "SERVER_PERFORMANCE":
                   if(isset($db_rec['ADMIN']))
                   {
                $to_email = $ADMIN_EMAIL; //adding case for performance mail
                   }
                   else
                   {
               $to_email = $db_rec['ADDITIONALEMAIL'];

                   }
                $key_arr = Array("#CONTENT#","#CURRENT#","#THRESHOLD#","#MAILFOOTER#");
                $val_arr = Array($db_rec['CONTENT'],$db_rec['CURRENT'],$db_rec['THRESHOLD'], $MAIL_FOOTER);
                break;
...
/*<div style="width:100%!important;margin-top:0;margin-bottom:0;margin-right:0;margin-left:0;padding-top:0;padding-bottom:0;padding-right:0;padding-left:0;font-family:Arial, Helvetica, sans-serif;">
  <table border="0" cellpadding="0" cellspacing="0" width="100%">
    ...
    function userwalletcurrency($currencyratio, $amount, $currencysymbol) {
        ...
        if ($amt == 0) {
           // $finalamt = $db_currency[0]['vSymbol'] . " 0.00";//codeEdit for remove currency symbol
             $finalamt = " 0.00";
            return $finalamt;
        } else {
            $finalamt = $db_currency[0]['vSymbol'] . ' ' . number_format($amt, $parameter, '.', '');
             $finalamt =  number_format($amt, $parameter, '.', '');
