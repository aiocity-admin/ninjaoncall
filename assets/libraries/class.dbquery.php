...
class DBConnection 
{
	...
	*/
	...
	{ 
		...
		//return $query;
		//echo $query;
		$results 	= mysqli_query($conn,$query) or die("Query1 failed: " . mysqli_error());
		$results 	= mysqli_insert_id($conn);
		if(!$results)
		   {
			 $this->MySQLDie("Query2 went bad!");
			 return false;
		   }
	    }
		...
		//echo $query;exit;
		$results = mysqli_query($conn,$query) or die("Query3 failed: " . mysqli_error());
		  if(!$results)
		  {
			 $this->MySQLDie("Query4 went bad!");
			 return false;
		  }
	    }
		return $results;
	...
	*/
	public function MySQLInsertPerform($table, $data, $action = 'insert', $parameters = '')
	{
		...
		$query 		= substr($query, 0, -2) . ')'; //Insert Query ready
		$results 	= mysqli_query($conn,$query) or die("Query6 failed: " . mysqli_error());
		if(!$results)
		   {
			 $this->MySQLDie("Query7 went bad!");
			 return false;
		   }
	    }
		return $results;
...
public function MyTokenId($tokenid)
	...
	{	
		if(empty($this->CONN)) { return false; }
		$conn = $this->CONN;
		$result_data = mysqli_query($conn,$string) or die('Query8 failed!');
	...
	public function GetInsertId(){
		if(empty($this->CONN)) { return false; }
		$conn = $this->CONN;
		$result = mysqli_insert_id($conn) or die('Query9 failed!');
