...
<!-- home page -->
    ...
    <!-- End: Left Menu-->
		...
		<div class="page-contant">
			<div class="page-contant-inner">
				...
				<div class="login-err">
				<span id="crossBtn" style="float:right;padding:6px 6px 0px 0;color: #fff;cursor:pointer;"></span>
				...
				<p style="display:none;" class="btn-block btn btn-rect btn-success error-login-v" id="success" ></p>
                        ...
                        <form action="" method="post" class="form-signin" id="frmforget" onSubmit="return forgotPass();" style="display: none;">
                            ...
                            <label><?=$langage_lbl['LBL_EMAIL_LBL_TXT']; ?></label>
								<input name="femail"  pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" type="email" placeholder="<?=$langage_lbl['LBL_EMAIL_LBL_TXT']; ?>" class="login-input" id="femail" value="" required />
							</b>
							<b>
								<input type="submit" id="recover" class="submit-but" value="<?=$langage_lbl['LBL_Recover_Password']; ?>" />
					...
					<div class="login-form-right login-form-right1">
						<?php if($APP_TYPE == 'Ride-Delivery-UberX' || $APP_TYPE == 'UberX') {  
							$sign_up_rider = 'sign-up-user';
						} else { 
							$sign_up_rider = 'sign-up-rider';
						} ?>
					<div class="login-form-right1-inner">
						      <h3><?=$langage_lbl['LBL_DONT_HAVE_ACCOUNT'];?></h3>
						      <span><a  class="company" href="<?=($action == 'rider')? $sign_up_rider:'sign-up';?>"><?=$langage_lbl['LBL_LOGIN_NEW_SIGN_UP'];?></a></span> 
				</div>
				<? if($iscompany == "0"){ ?>
				<div class="login-form-right1-inner">
				<?php ?>
    ...
    <script>
		...
		function chkValid(login_type)
		{
			...
			{
				document.getElementById("crossBtn").innerHTML = '<i class="fa fa-times"></i>';
				var request = $.ajax({
					type: "POST",
