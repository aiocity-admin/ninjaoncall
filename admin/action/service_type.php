...
if ($method == 'delete' && $iVehicleTypeId != '') {
	if(SITE_TYPE !='Demo'){
            ...
            $obj->sql_query($query);
            $_SESSION['success'] = '1';
            $_SESSION['var_msg'] = 'Service Type deleted successfully.';   
	}
	else{
            $_SESSION['success'] = '2';
	}
	header("Location:".$tconfig["tsite_url_main_admin"]."service_type.php?".$parameters); exit;
}
//End make deleted
...
if($checkbox != "" && $statusVal == "Deleted") {
	if(SITE_TYPE !='Demo'){
		 ...
		 $obj->sql_query($query);
		 $_SESSION['success'] = '1';
		 $_SESSION['var_msg'] = 'Service Type(s) deleted successfully.';
	}
	else{
		$_SESSION['success']=2;
	}
