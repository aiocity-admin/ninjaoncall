...
if ($iVehicleCategoryId != '' && $status != '') {
	if(SITE_TYPE !='Demo'){
            ...
            $_SESSION['success'] = '1';
            if($status == 'Active') {
                   $_SESSION['var_msg'] = 'Service Sub Category activated successfully.';
            }else {
                   $_SESSION['var_msg'] = 'Service Sub Category inactivated successfully.';
            }
	}
	else{
            $_SESSION['success']=2;
	...
	if(SITE_TYPE !='Demo'){
		if($statusVal == "Deleted") {
			...
			$obj->sql_query($query);
		 	...
		 	$obj->sql_query($query);
		 	$_SESSION['success'] = '1';
		 	$_SESSION['var_msg'] = 'Service Sub Category(s) updated successfully.';
		}
