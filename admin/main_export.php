...
if ($section == 'rider') {
    ...
    $sql = "SELECT CONCAT(vName,' ',vLastName) as `User Name`,vEmail as Email,tRegistrationDate as `Sign Up Date`,CONCAT(vPhoneCode,' ',vPhone) AS Mobile,eStatus as Status FROM register_user WHERE 1=1 $eStatus_sql $ssql $ssql1 $rdr_ssql $ord";
    ...
    if ($type == 'XLS') {
        ...
        $heading = array('User Name', 'Email','Sign Up Date', 'Mobile', 'Status');
        ...
        foreach ($heading as $column_heading) {
           ...
           if ($column_heading == 'Email') {
            ...
            } else if ($column_heading == 'Sign Up Date') {
...
if ($section == 'vehicle_type') {
    ...
    if($Vehicle_type_name == "Ride-Delivery") {
        ...
        $sql = "SELECT vt.vVehicleType_".$default_lang." as Type,vt.fPricePerKM as PricePer".$DEFAULT_DISTANCE_UNIT.",vt.fPricePerMin as PricePerMin,vt.iBaseFare as BaseFare,vt.ServiceFeeType as `Service fees type`,vt.fCommision as `Revenue Fees`,vt.FixedCommission as `Fixed Fees`,vt.iPersonSize as PersonSize,vt.eType as `Category`, lm.vLocationName as location, vt.iLocationid as locationId from  vehicle_type as vt left join location_master as lm ON lm.iLocationId = vt.iLocationid where 1=1 $ssql $ord";
    ...
    if ($type == 'XLS') {
        ...
        foreach($result as $value){
            ...
            foreach($value as $key=>$val) {
                ...
                if($key=='Category' && $value['Category'] == 'Deliver')
                {
 $val = "Delivery";
   ...
   if($key == 'Service fees type'){

    switch ($value['Service fees type']) {
    case '1':
       
       $val =   'Revenue Share Only';
   ...
   /* case '3':
       $val =  'Token Fee Only';
        break;*/
           case '4':
        # code...
            $val =  'Revenue Share and Fixed Fee';
        break;
          /* case '5':
        # code...
            $val =  'Revenue Share and Token Fee';
        break;   */    
        ...
        if($APP_TYPE == 'UberX') {
            ...
            if($Vehicle_type_name == "Ride-Delivery") {
                ...
                $heading = array('Type','PricePer'.$DEFAULT_DISTANCE_UNIT,'PricePerMin','BaseFare','Service fees type','Revenue Fees','Fixed Fees','PersonSize','Category','Location Name');

            } else {

                $heading = array('Type','PricePer'.$DEFAULT_DISTANCE_UNIT,'PricePerMin','BaseFare','Service fees type','Revenue Fees','Fixed Fees','PersonSize','Location Name');
        ...
        foreach ($heading as $column_heading) {
            ...
            if ($column_heading == 'Type' && $APP_TYPE == 'UberX') {
            ...
            else if ($column_heading == 'Revenue Fees') {

                $pdf->Cell(23, 8, $column_heading, 1);

            } else if ($column_heading == 'Fixed Fees') {

                $pdf->Cell(18, 8, $column_heading, 1);

            } else if ($column_heading == 'Category') {

                $pdf->Cell(15, 8, $column_heading, 1);

            }else if ($column_heading == 'Service fees type') {

                $pdf->Cell(27, 8, $column_heading, 1);
                ...
                $pdf->Cell(24, 8, $column_heading, 1);

            } else if ($column_heading == 'Subcategory') {

                $pdf->Cell(30, 8, $column_heading, 1);

            } else {

                $pdf->Cell(20, 8, $column_heading, 1); 
        ...
        foreach ($result as $row) {
            ...
            foreach ($row as $column => $key) {
               ...
               if ($column == 'Type' && $APP_TYPE == 'UberX') {
                ...
                } else if ($column == 'Revenue Fees') {

                $pdf->Cell(23, 8, $key, 1);

            } else if ($column == 'Fixed Fees') {

                $pdf->Cell(18, 8, $key, 1);

            }else if ($column == 'Category'  && $row['Category'] == "Deliver") {

                $pdf->Cell(15, 8, 'Delivery', 1);

            }else if ($column == 'Category') {

                $pdf->Cell(15, 8, $key, 1);

            } else if ($column == 'Service fees type') {
...
if ($section == 'service_type') {
   ...
   // $Vehicle_type_name = ($APP_TYPE == 'Delivery')? 'Deliver':$APP_TYPE ;
    ...
    $sql = "SELECT vt.vVehicleType_".$default_lang." as Type,vcParent.vCategory_".$default_lang." as `Service category`,vc.vCategory_".$default_lang." as `Sub category`,vt.iDisplayOrder as `Display Order`,lm.vLocationName as Localization,vt.iLocationid as locationId from vehicle_type as vt  left join vehicle_category as vc on vt.iVehicleCategoryId = vc.iVehicleCategoryId left join country as c ON c.iCountryId = vt.iCountryId left join state as st ON st.iStateId = vt.iStateId left join city as ct ON ct.iCityId = vt.iCityId left join location_master as lm ON lm.iLocationId = vt.iLocationid left join vehicle_category vcParent on  vc.iParentId=vcParent.iVehicleCategoryId where vt.eType='".$Vehicle_type_name."'  $ssql $ord";  
    ...
    if ($type == 'XLS') {
        ...
        foreach($result as $value){
            ...
            foreach($value as $key=>$val) {
                ...
                if($key == 'Localization' && $value['locationId'] == '-1'){
        ...
        if($Vehicle_type_name == 'UberX') {
            ...
            $heading = array('Type','Service category','Sub category','Display Order','Localization');
        ...
        foreach ($heading as $column_heading) {
            ...
            if ($column_heading == 'Type' && $Vehicle_type_name == 'UberX') {
                ...
                $pdf->Cell(57, 8, $column_heading, 1);

            } else if ($column_heading == 'Localization') {

                $pdf->Cell(35, 8, $column_heading, 1);

            }else if ($column_heading == 'Service category') {

                    $pdf->Cell(40, 8, $column_heading, 1);

                } else if ($column_heading == 'Sub category') {

                $pdf->Cell(45, 8, $column_heading, 1);

            } else if ($column_heading == 'Display Order') {

                $pdf->Cell(25, 8, $column_heading, 1);

            }  else {

                $pdf->Cell(26, 8, $column_heading, 1); 
        ...
        foreach ($result as $row) {
            ...
            foreach ($row as $column => $key) {
               ...
               if ($column == 'Type' && $Vehicle_type_name == 'UberX') {
                ...
                }else if ($column == 'Localization' && $row['locationId'] == "-1") {

                    $pdf->Cell(35, 8, 'All Location', 1);
...
if ($section == 'driver') {
    ...
    $sql = "SELECT CONCAT(rd.vName,' ',rd.vLastName) AS `Driver Name`,c.vCompany as `Company Name`,rd.vEmail as `Email Id`, rd.tRegistrationDate as `Signup Date`,CONCAT(rd.vCode,' ',rd.vPhone) as `Phone`,rd.eStatus as `status` FROM register_driver rd LEFT JOIN company c ON rd.iCompanyId = c.iCompanyId 
    ...
    if ($type == 'XLS') {
        ...
        $heading = array($langage_lbl_admin['LBL_DRIVER_NAME_EXPORT'],'Company Name','Email Id','Signup Date','Phone','Status');
...
if ($section == 'vehicles') {
  ...
  $ssql.= " AND (".stripslashes($option)." LIKE '%".$generalobjAdmin->clean($keyword)."%' or md.vTitle"." LIKE '%".$generalobjAdmin->clean($keyword)."%' )  AND dv.eStatus = '".$generalobjAdmin->clean($eStatus)."'";
            else if($option=="provider")
          $ssql.= " AND (CONCAT(rd.vName,' ',rd.vLastName) LIKE '%".$generalobjAdmin->clean($keyword)."%' or md.vTitle"." LIKE '%".$generalobjAdmin->clean($keyword)."%' )  AND dv.eStatus = '".$generalobjAdmin->clean($eStatus)."'";
...
if($option=="m.vMake")
            $ssql.= " AND (".stripslashes($option)." LIKE '%".$generalobjAdmin->clean($keyword)."%' or md.vTitle"." LIKE '%".$generalobjAdmin->clean($keyword)."%' )";
            else if($option=="provider")
          $ssql.= " AND (CONCAT(rd.vName,' ',rd.vLastName) LIKE '%".$generalobjAdmin->clean($keyword)."%' or md.vTitle"." LIKE '%".$generalobjAdmin->clean($keyword)."%' )";
            ...
            if($eStatus != ''){
                ...
                $ssql.= " AND (vt.vVehicleType LIKE '%".$generalobjAdmin->clean($keyword)."%' or md.vTitle LIKE '%".$generalobjAdmin->clean($keyword)."%' or m.vMake LIKE '%".$keyword."%' OR c.vCompany LIKE '%".$keyword."%' OR CONCAT(rd.vName,' ',rd.vLastName) LIKE '%".$keyword."%')  AND dv.eStatus = '".$generalobjAdmin->clean($eStatus)."'";

            } else {

                $ssql.= " AND (vt.vVehicleType LIKE '%".$generalobjAdmin->clean($keyword)."%' or md.vTitle LIKE '%".$generalobjAdmin->clean($keyword)."%' or m.vMake LIKE '%".$keyword."%' OR c.vCompany LIKE '%".$keyword."%' OR CONCAT(rd.vName,' ',rd.vLastName) LIKE '%".$keyword."%')";
    ...
    if($APP_TYPE == 'UberX'){
        ...
        $sql = "SELECT dv.iDriverVehicleId,dv.eStatus,CONCAT(rd.vName,' ',rd.vLastName) AS driverName,dv.vLicencePlate, c.vCompany FROM driver_vehicle dv, register_driver rd,company c,vehicle_type vt

            WHERE 1 = 1   AND dv.vCarType LIKE CONCAT('%',vt.iVehicleTypeId,'%')  and dv.iDriverId = rd.iDriverId  AND dv.iCompanyId = c.iCompanyId $eStatus_sql $ssql $dri_ssql";

    } else {

        if($APP_TYPE == 'Ride-Delivery-UberX' || $APP_TYPE == 'Ride-Delivery'){

            $sql = "SELECT  CONCAT(m.vMake,' ', md.vTitle) AS Vehicle, c.vCompany AS Company, CONCAT(rd.vName,' ',rd.vLastName) AS Driver ,dv.vCarType as `Sevice Type`,dv.eType as `Sevice Category`, dv.eStatus as Status FROM driver_vehicle dv, register_driver rd, make m, model md, company c,vehicle_type vt WHERE 1 = 1 AND dv.vCarType LIKE CONCAT('%',vt.iVehicleTypeId,'%')  and dv.iDriverId = rd.iDriverId AND dv.iCompanyId = c.iCompanyId AND dv.iModelId = md.iModelId AND dv.iMakeId = m.iMakeId $eStatus_sql $ssql $dri_ssql $ord ";//LIMIT $start, $per_page

        } else {

           $sql = "SELECT  CONCAT(m.vMake,' ', md.vTitle) AS Vehicle, c.vCompany AS Company, CONCAT(rd.vName,' ',rd.vLastName) AS Driver,dv.vCarType as `Sevice Category` ,dv.eStatus as Status FROM driver_vehicle dv, register_driver rd, make m, model md, company c,vehicle_type vt WHERE 1 = 1 AND dv.vCarType LIKE CONCAT('%',vt.iVehicleTypeId,'%')  and dv.iDriverId = rd.iDriverId AND dv.iCompanyId = c.iCompanyId AND dv.iModelId = md.iModelId AND dv.iMakeId = m.iMakeId $eStatus_sql $ssql $dri_ssql $ord ";//LIMIT $start, $per_page 
    ...
    if ($type == 'XLS') {
     ...
     if($key == 'Sevice Type'){
        ...
        if($APP_TYPE == 'Ride-Delivery-UberX' || $APP_TYPE == 'Ride-Delivery'){
            ...
            $heading = array('Vehicle','Company',$langage_lbl_admin['LBL_DRIVER_TXT_ADMIN'],'Sevice Type','Sevice Category','Status');

        } else {

            $heading = array('Vehicle','Company',$langage_lbl_admin['LBL_DRIVER_TXT_ADMIN'],'Status');
        ...
        foreach ($heading as $column_heading) {
            ...
            }   else if ($column_heading == 'Sevice Category') {

                $pdf->Cell(30, 10, $column_heading, 1);

            }

            else if ($column_heading == 'Sevice Type') {
        ...
        foreach ($result as $row) {
            ...
            foreach ($row as $column => $key) {
                ...
                if ($column == 'Vehicle') {
                    ...
                    $pdf->Cell(35, 10, $generalobjAdmin->clearCmpName($key), 1);

                }   else if ($column == 'Driver') {                 

                  $pdf->Cell(35, 10, $generalobjAdmin->clearName($key), 1); //}

                } else if ($column == 'Sevice Category') {

                $pdf->Cell(30, 10, $key, 1);

            }else if ($column == 'Sevice Type') {

                    $pdf->Cell(30, 10, getServiceType($key), 1);
...
if ($section == 'sub_service_category') {   
    ...
    if($parent_ufx_catid != "0") {
        ...
        $sql = "SELECT vCategory_".$default_lang." as Name, (SELECT vCategory_".$default_lang." FROM vehicle_category WHERE iVehicleCategoryId='".$sub_cid."') as Category, (select count(iVehicleTypeId) from vehicle_type where vehicle_type.iVehicleCategoryId = vehicle_category.iVehicleCategoryId) as `Service Types`, iDisplayOrder as `Display Order`, eStatus as Status FROM vehicle_category WHERE 1 = 1 $ssql $ord";
    ...
    if ($type == 'XLS') {
        ...
        $heading = array('Name', 'Category' ,'Service Types','Display Order','Status');
...
if ($section == 'service_category') {
   ...
   $sql = "SELECT vc.vCategory_".$default_lang." as Name ,(select count(iVehicleCategoryId) from vehicle_category where iParentId=vc.iVehicleCategoryId) as `Sub Categories`,vc.iDisplayOrder as `Display Order`,vc.eStatus as Status FROM vehicle_category as vc WHERE  vc.iParentId='0' $ssql $ord"; 
    ...
    if ($type == 'XLS') {
        ...
        $heading = array('Name','Sub Categories','Display Order','Status');
...
if ($section == 'Document_Master') {
   ...
   $ord = ' ORDER BY dm.doc_name ASC';
    ...
    if($APP_TYPE == 'Ride-Delivery' || $APP_TYPE == 'Ride-Delivery-UberX') {
    	...
    	$sql = "SELECT if(c.vCountry IS NULL,'All',c.vCountry) as Country,dm.doc_name as `Document Name`,dm.doc_usertype as `Document For`, dm.status as Status, dm.eType as `Service Type` FROM `document_master` AS dm
    ...
    if ($type == 'XLS') {
        ...
        if($APP_TYPE == 'Ride-Delivery' || $APP_TYPE == 'Ride-Delivery-UberX') {
            ...
            $heading = array('Country','Document Name','Document For','Status','Service Type');
...
function getServiceType($value)
{
 ...
 { 
 $sql="SELECT * FROM `vehicle_type` where `iVehicleTypeId`='".$value[$i]."'";

 $data = $obj->MySQLSelect($sql);
 if (trim($data[0]['vVehicleType'])!="") {
  
 
$serviceTypes.=" ".$data[0]['vVehicleType'].",";
}
 }
$serviceTypes=rtrim($serviceTypes,",");
