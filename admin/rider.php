...
if($keyword != ''){
    ...
    if($option != '') {
        $option_new = $option;
        if($option == 'RiderName'){
          $option_new = "CONCAT(Suffix,'',vName,'',MiddleName,'',vLastName)";
        }
        ...
        if($eStatus != ''){
            $ssql.= " AND (CONCAT(Suffix,'',vName,'',MiddleName,'',vLastName) LIKE '%".$generalobjAdmin->clean($keyword_new)."%' OR vEmail LIKE '%".$generalobjAdmin->clean($keyword_new)."%' OR (CONCAT(vPhoneCode,'',vPhone) LIKE '%".$generalobjAdmin->clean($keyword_new)."%')) AND eStatus = '".$generalobjAdmin->clean($eStatus)."'";
        } else {
            $ssql.= " AND (CONCAT(Suffix,'',vName,'',MiddleName,'',vLastName) LIKE '%".$generalobjAdmin->clean($keyword_new)."%' OR vEmail LIKE '%".$generalobjAdmin->clean($keyword_new)."%' OR (CONCAT(vPhoneCode,'',vPhone) LIKE '%".$generalobjAdmin->clean($keyword_new)."%'))";
        }
    }
...
if(!empty($eStatus)) { 
    ...
    $sql = "SELECT iUserId,CONCAT(Suffix,' ',vName,' ',MiddleName,' ',vLastName) AS name, vEmail, vPhone AS mobile,vPhoneCode,tRegistrationDate,eStatus FROM register_user WHERE 1=1 $esql $ssql $ssql1 $rdr_ssql $ord LIMIT $start, $per_page";
   // echo $sql;
...
<!--  codeEdited for remove wallet balance add balance button														 <th  class="align-Left">Wallet Balance</th>
 -->                                                        <th width="8%" align="center" style="text-align:center;"><a href="javascript:void(0);" onClick="Redirect(4,<?php if($sortby == '4'){ echo $order; }else { ?>0<?php } ?>)">Status <?php if ($sortby == 4) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
                                                ...
                                                <tbody>
													<?php if(!empty($data_drv)) { 
														...
														<!--  codeEdited for remove wallet balance add balance button	<td>
