...
date1= date1.getFullYear() + '-' +
 ...
 $.ajax({
           ...
           data: {"loadtime":seconds,"beforeload":date1,"afterload":date2,"UserType":"SUPER_ADMIN","eType":"DRIVER"}, 
...
if ($_POST['action'] =='adjustmoney') 
{
   ...
   $eUserType = $_REQUEST['eUserType'];
    ...
    $eType = $_REQUEST['eType'];
    $iTripId = 0;
    $tDescription = '#LBL_ADJUSTED_BY_ADMIN#';  
    $ePaymentStatus = 'Unsettelled';
    $dDate = Date('Y-m-d H:i:s');
    ...
    $_SESSION['success'] = '1';
    $_SESSION['var_msg'] = $langage_lbl_admin["LBL_DRIVER_TXT_ADMIN"].' in adjust balance successfully';  
      header("Location:".$tconfig["tsite_url_main_admin"]."driver.php"); exit;
}
...
/*    if($execute) {
   ...
   // $keyword_new = trim(str_replace($chracters, "", $removespacekeyword));
    ...
    if($option != '') {
      ...
      if($eStatus != ''){
        $ssql.= " AND (concat(rd.vName,' ',rd.vLastName) LIKE '%".$generalobjAdmin->clean($keyword_new)."%' OR c.vCompany LIKE '%".$keyword_new."%' OR rd.vEmail LIKE '%".$generalobjAdmin->clean($keyword_new)."%' OR (concat(rd.vCode,'',rd.vPhone) LIKE '%".$generalobjAdmin->clean($keyword_new)."%')) AND rd.eStatus = '".$generalobjAdmin->clean($eStatus)."'";
      } else {
...
if($servicetype!="")
{
 if(trim($servicetype)=='Ride'||trim($servicetype)=='Delivery')
{
    $ssl_servicetype=" and dv.eType ='$servicetype' ";

}
else{
  $ssl_servicetype=" and dv.vCarType like concat('%',$servicetype,'%') ";

}
$join_servicetype="  left join driver_vehicle dv on rd.iDriverId=dv.iDriverId left join vehicle_type vt on  dv.vCarType like concat('%',vt.iVehicleTypeId,'%') ";
$GROUPbY_servicetype=" group by rd.iDriverId ";

$sql = "select COUNT(Total) as Total from ( SELECT rd.iDriverId AS Total FROM register_driver rd LEFT JOIN company c ON rd.iCompanyId = c.iCompanyId $join_servicetype WHERE 1 = 1  $eStatussql $ssql $ssql1 $dri_ssql $ssl_servicetype $GROUPbY_servicetype ) t";

}else{

  $sql = "SELECT COUNT(rd.iDriverId) AS Total FROM register_driver rd LEFT JOIN company c ON rd.iCompanyId = c.iCompanyId WHERE 1 = 1  $eStatussql $ssql $ssql1 $dri_ssql";
}
...
if($servicetype!="")
{
  ...
  $sql = "SELECT rd.iDriverId,rd.vEmail,rd.tRegistrationDate,rd.vPhone,rd.vCode,rd.eStatus,(SELECT count(dv.iDriverVehicleId) FROM driver_vehicle AS dv WHERE dv.iDriverId=rd.iDriverId AND dv.eStatus != 'Deleted' AND dv.iMakeId != 0 AND dv.iModelId != 0 AND dv.eType != 'UberX') AS `count`,CONCAT(rd.vName,' ',rd.vLastName) AS driverName, c.vCompany,c.eStatus as cmp_status FROM register_driver rd LEFT JOIN company c ON rd.iCompanyId = c.iCompanyId $join_servicetype WHERE 1=1 $eQuery $ssql $ssql1 $dri_ssql $ssl_servicetype $GROUPbY_servicetype $ord LIMIT $start, $per_page";
}else{
      ...
      <div id="wrap">
          ...
          <!--PAGE CONTENT -->
            ...
            <input type="hidden" name="iDriverId" value="<?php echo $iDriverId; ?>" >
               ...
               <th  class="align-Left">Wallet Tokens</th>
                            <?php if($doc_count != 0) { ?>
                          ...
                          ?>
                            <tr class="gradeA" >
                             <td align="center"><input type="checkbox" id="checkbox" name="checkbox[]" <?php echo $default; ?> value="<?php echo $data_drv[$i]['iDriverId']; ?>" />&nbsp;</td>
                              ...
                              <td style="word-break: break-word;"><?= $generalobjAdmin->clearEmail($data_drv[$i]['vEmail']);?></td>
                              <?php if($APP_TYPE != "UberX"){
                              ...
                              if($data_drv[$i]['eStatus'] != "Deleted"){ 
                                ...
                                ?>
                              <button type="button" onClick="Add_money_driver('<?=$data_drv[$i]['iDriverId'];?>')" class="btn btn-success btn-xs">Add Tokens</button> <br>
                                <input type="button" name="AdjustTokens" id="AdjustTokens" onclick="Adjust_money_driver('<?php echo $data_drv[$i]['iDriverId'];?>')"  class="btn btn-success btn-xs AdjustTokens" value="Adjust Tokens">
                              <?php }else{
                                echo $generalobj->trip_currency($user_available_balance);
                                } ?>
                              ...
                              <td align="center">
                                ...
                                /*  $sql = "select iDriverVehicleId from register_driver where iDriverId = '" . $data_drv[$i]['iDriverId'] . "' ";
                                  ...
                                  $newUrl2 = "manage_service_type.php?iDriverId=".$data_drv[$i]['iDriverId']."";
                                }
                                ?>
                                <?php if($data_drv[$i]['eStatus']!="Deleted"){?> 
                ...
                <div class="col-lg-12">
                  <div class="input-group input-append" >
                    <h5><?= $langage_lbl['LBL_ADD_WALLET_DESC_TXT']; ?></h5>
                                        <div class="ddtt">
                    <h4>Enter tokens</h4>
                    <input type="text" maxlength="5" name="iBalance" id="iBalance" class="form-control iBalance add-ibalance" onKeyup="checkzero(this.value);">
                    </div>
  ...
  <div  class="modal fade" id="driver_adjust_wallet_money" tabindex="-1" role="dialog"  aria-hidden="true" >
                         <div class="modal-dialog" >
                              <div class="modal-content ">
                                   ...
                                   <br>
                                   <input type="submit" class="btn btn-primary btn-adjust" name="btnAdjustTokens" id="btnAdjustTokens" value="save"/>
