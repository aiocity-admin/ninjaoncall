...
error_reporting(E_ALL);
	...
	//echo '<pre>'; print_R($db_data); echo '</pre>';
		...
		global $obj;
		$sql1 = "SELECT count(*) as total,vstate FROM state where iCountryId='".$id."'";
		$db_data1 = $obj->MySQLSelect($sql1);
		
		echo "<a href='state.php?country_id=".$id."'>(".$db_data1[0]['total'].")</a>";
													...
													<?
														...
														if($count_all > 0) {
															for($i=0;$i<$count_all;$i++) {
																...
																$vCountryCodeISO_3 = $db_data[$i]['vCountryCodeISO_3'];
																$eStatus = $db_data[$i]['eStatus'];
																//$statename = state_name($id);
																$checked = ($eStatus=="Active")?'checked':'';
															?>
															<tr class="gradeA">
																<td width="30%"><?=$vCountry;?></td>
																<td width="10%"><?=state_detail($id); ?></td>
																<td width="10%"><?=$vPhoneCode;?></td>
																<td width="10%"><?=$vCountryCodeISO_3;?></td>
