<?
	...
	/*****************************/
	$sql = "SELECT count(iDriverId) as TotalDriver FROM register_driver WHERE 1 AND (vEmail != '' OR vPhone != '') AND tRegistrationDate BETWEEN '".date('Y-m', strtotime(date('Y-m-d')." -2 month"))."-"."01"." 00:00:00' AND '".date('Y-m', strtotime(date('Y-m-d')." -2 month"))."-"."31"." 23:59:59'";
	$driver_2 = $obj->MySQLSelect($sql);
	
	$sql = "SELECT count(iDriverId) as TotalDriver1 FROM register_driver WHERE 1 AND (vEmail != '' OR vPhone != '') AND tRegistrationDate BETWEEN '".date('Y-m', strtotime(date('Y-m-d')." -1 month"))."-"."01"." 00:00:00' AND '".date('Y-m', strtotime(date('Y-m-d')." -1 month"))."-"."31"." 23:59:59'";
	$driver_1 = $obj->MySQLSelect($sql);
	
	$sql = "SELECT count(iDriverId) as TotalDriver2 FROM register_driver WHERE 1 AND (vEmail != '' OR vPhone != '') AND tRegistrationDate BETWEEN '".date('Y-m', strtotime(date('Y-m-d')))."-"."01"." 00:00:00' AND '".date('Y-m', strtotime(date('Y-m-d')))."-"."31"." 23:59:59'";
	$driver_0 = $obj->MySQLSelect($sql);
	/*****************************/
	
	/*****************************/
	$sql = "SELECT count(iUserId) as TotalRider FROM register_user WHERE 1 AND (vEmail != '' OR vPhone != '') AND tRegistrationDate BETWEEN '".date('Y-m', strtotime(date('Y-m-d')." -2 month"))."-"."01"." 00:00:00' AND '".date('Y-m', strtotime(date('Y-m-d')." -2 month"))."-"."31"." 23:59:59'";
	$pass_2 = $obj->MySQLSelect($sql);
	
	$sql = "SELECT count(iUserId) as TotalRider1 FROM register_user WHERE 1 AND (vEmail != '' OR vPhone != '') AND tRegistrationDate BETWEEN '".date('Y-m', strtotime(date('Y-m-d')." -1 month"))."-"."01"." 00:00:00' AND '".date('Y-m', strtotime(date('Y-m-d')." -1 month"))."-"."31"." 23:59:59'";
	$pass_1 = $obj->MySQLSelect($sql);
	
	$sql = "SELECT count(iUserId) as TotalRider2 FROM register_user WHERE 1 AND (vEmail != '' OR vPhone != '') AND tRegistrationDate BETWEEN '".date('Y-m', strtotime(date('Y-m-d')))."-"."01"." 00:00:00' AND '".date('Y-m', strtotime(date('Y-m-d')))."-"."31"." 23:59:59'";
...
<!--[if !IE]><!--> <html lang="en"> <!--<![endif]-->
	...
	<head>
		<meta charset="UTF-8" />
		<title>Admin | Site Statistics</title>
...
<h3><?php echo $langage_lbl_admin['LBL_DRIVERS_NAME_ADMIN'];?>  Count : <?=number_format($actDrive[0]['tot_driver']+$inaDrive[0]['tot_driver']);?></h3>
...
<script>
	$(document).ready(function(){
		...
		Morris.Bar({
			...
			labels: ['Generated Fare', 'Discount','Wallet','Paid By User','Paid In Cash']
		});
