...
if($keyword != ''){
    if($option != '') {
       if($eStatus != ''){
        ...
        if($eStatus != ''){
            $ssql.= "AND (CONCAT(m.vMake,' ',md.vTitle) LIKE '%".$generalobjAdmin->clean($keyword)."%' or  vt.vVehicleType LIKE '%".$generalobjAdmin->clean($keyword)."%' or md.vTitle LIKE '%".$generalobjAdmin->clean($keyword)."%' or m.vMake LIKE '%".$generalobjAdmin->clean($keyword)."%' OR c.vCompany LIKE '%".$generalobjAdmin->clean($keyword)."%' OR CONCAT(rd.vName,' ',rd.vLastName) LIKE '%".$generalobjAdmin->clean($keyword)."%') AND dv.eStatus = '".$generalobjAdmin->clean($eStatus)."'";
        } else {
            $ssql.= " AND (CONCAT(m.vMake,' ',md.vTitle) LIKE '%".$generalobjAdmin->clean($keyword)."%' or vt.vVehicleType LIKE '%".$generalobjAdmin->clean($keyword)."%' or md.vTitle LIKE '%".$generalobjAdmin->clean($keyword)."%' or m.vMake LIKE '%".$generalobjAdmin->clean($keyword)."%' OR c.vCompany LIKE '%".$generalobjAdmin->clean($keyword)."%' OR CONCAT(rd.vName,' ',rd.vLastName) LIKE '%".$generalobjAdmin->clean($keyword)."%')";
...
if(!empty($eStatus)){
   $eStatussql = "";
    ...
    $eStatussql = " AND dv.eStatus != 'Deleted' AND dv.eType != 'UberX'";
}
if($APP_TYPE == 'UberX'){
    $sql = "SELECT COUNT(iDriverVehicleId) AS Total from ( select  dv.vLicencePlate,dv.iDriverVehicleId FROM driver_vehicle AS dv, register_driver rd, make m, model md, company c WHERE 1=1 AND dv.iDriverId = rd.iDriverId  AND dv.iCompanyId = c.iCompanyId AND dv.eType != 'UberX'".$eStatussql.$ssql.$dri_ssql." GROUP BY dv.vLicencePlate) x";
} else {
    $sql = "SELECT COUNT(iDriverVehicleId) AS Total from ( select  dv.vLicencePlate,dv.iDriverVehicleId FROM driver_vehicle AS dv, register_driver rd, make m, model md, company c WHERE 1=1 AND dv.iDriverId = rd.iDriverId AND dv.iCompanyId = c.iCompanyId AND dv.iModelId = md.iModelId  AND dv.iMakeId = m.iMakeId AND dv.eType != 'UberX'".$eStatussql.$ssql.$dri_ssql." GROUP BY dv.vLicencePlate) x" ;
...
if(!empty($eStatus)){
    ...
    $eQuery= " AND dv.eStatus != 'Deleted' AND dv.eType != 'UberX'";
}
if($APP_TYPE == 'UberX'){
   $sql = "SELECT dv.iDriverVehicleId,dv.eStatus,CONCAT(rd.vName,' ',rd.vLastName) AS driverName,dv.vLicencePlate, c.vCompany, dv.eType,dv.vLicencePlate,dv.vCarType FROM driver_vehicle dv, register_driver rd,company c,vehicle_type vt WHERE 1=1 AND dv.vCarType LIKE CONCAT('%',vt.iVehicleTypeId,'%')  and dv.iDriverId = rd.iDriverId  AND dv.iCompanyId = c.iCompanyId AND dv.eType != 'UberX'  $eQuery $ssql  $dri_ssql GROUP BY dv.vLicencePlate";
} else {
	$sql = "SELECT dv.iDriverVehicleId, dv.iDriverId, dv.eStatus, m.vMake, md.vTitle,CONCAT(rd.vName,' ',rd.vLastName) AS driverName, c.vCompany, dv.eType,dv.vLicencePlate,dv.vCarType FROM driver_vehicle dv, register_driver rd, make m, model md, company c,vehicle_type vt WHERE 1=1 AND dv.vCarType LIKE CONCAT('%',vt.iVehicleTypeId,'%')  and dv.iDriverId = rd.iDriverId AND dv.iCompanyId = c.iCompanyId AND dv.iModelId = md.iModelId AND dv.iMakeId = m.iMakeId AND dv.eType != 'UberX' $eQuery $ssql $dri_ssql  GROUP BY dv.vLicencePlate $ord  LIMIT $start, $per_page";
}
//echo $sql;
...
<html lang="en">
    ...
    <head>
        <meta charset="UTF-8" />
        <title><?=$SITE_NAME?> | <?php echo $langage_lbl_admin['LBL_VEHICLE_TXT_ADMIN']; ?> </title>
        ...
        <div id="wrap">
            ...
            <div id="content">
                <div class="inner">
                  ...
                  $drv_text = ($drv_name != "") ? "Vehicles of ".$generalobjAdmin->clearName($drv_name) : $langage_lbl_admin['LBL_VEHICLE_TXT_ADMIN'];
								?>
                                <h2><?=$drv_text?></h2>
                            </div>
                        </div>
                              ...
                              <tbody>
                                <tr>
                                    ...
                                    <select name="option" id="option" class="form-control">
                                          ...
                                          <option  value="CONCAT(m.vMake,' ',md.vTitle)" <?php if ($option == "CONCAT(m.vMake,' ',md.vTitle)"){ echo "selected"; } ?> >Vehilce</option>
                                          <option value="c.vCompany" <?php if ($option == 'c.vCompany') {echo "selected"; } ?> >Company</option>
                                          <option value="provider" <?php if ($option=="provider" || $option == "CONCAT(rd.vName,' ',rd.vLastName)" || ($iDriverId !="" && $drv_name != "")) {echo "selected"; } ?> >Provider</option>
                                          <? if($APP_TYPE == 'Ride-Delivery-UberX' || $APP_TYPE == 'Ride-Delivery') {?>
                                          <option value="dv.eType" <?php if ($option == 'dv.eType') {echo "selected"; } ?> >Service Category</option>
                                          <? } ?>
                                           <option value="vt.vVehicleType" <?php if ($option == 'vt.vVehicleType') {echo "selected"; } ?> >Service Type</option>
                                    ...
                                    <td width="12%" class="eType_options" id="eType_options" >
                                        <select name="eType" id="eType_value" class="form-control">
                                            ...
                                            <option value='Delivery' <?php if ($eType == 'Delivery') {echo "selected"; } ?> >Delivery</option>
                                        </select>
                                    </td>
                                    ...
                                    <td width="25%"><a class="add-btn" href="vehicle_add_form.php" style="text-align: center;">ADD VEHICLES</a></td>
                                </tr>
                              </tbody>
                        </table>
                                            ...
                                            <table class="table table-striped table-bordered table-hover">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        if($APP_TYPE == 'Ride-Delivery-UberX' || $APP_TYPE == 'Ride-Delivery'){ 
                                                        ?>
                                                        <th width="8%"><a href="javascript:void(0);" onClick="Redirect(4,<?php if($sortby == '4'){ echo $order; }else { ?>0<?php } ?>)">Service Category<?php if ($sortby == 4) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th> 
                                                        <? } ?>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr class="gradeA">
                                                        ...
                                                        <td><?php echo getServiceType($data_drv[$i]['vCarType']);?></td>
                                                      <?php if($doc_count != 0) {
                                                        if($APP_TYPE != 'UberX'){ ?> 
