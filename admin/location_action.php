...
function IsEmpty(){
  ...
  function initialize() {
	...
	//codeEdited for set default zoom on Philippines
	var myLatLng = new google.maps.LatLng("<?php if(trim($latitude)=='') echo '12.879721'; else echo $latitude;?>","<?php if (trim($longitude)=='') echo '121.77401699999996'; else echo $longitude;?>");
...
//https://nominatim.openstreetmap.org/search.php?q=NOIDA&polygon_geojson=1&format=json
        marker.setIcon(({
            url: place.icon,
        ...
        //codeEdited for auto location
var serachAddress="";
var z=0;
var arr_formatted_address=place.formatted_address.trim().replace(/,/g, '').replace(/Brgy./g,'Barangay').split(" ");
for (var i = 0; i < arr_formatted_address.length; i++) {
	//if (i!=0) {
	//if(arr_formatted_address!=place.address_components[i-1].long_name)
	//{
		//if (isNaN(place.address_components[i].long_name))
		//{
	//serachAddress+="+"+place.address_components[i].long_name.replace(/ /g, '+');z++;
//}
//} else{
	if (isNaN(arr_formatted_address[i].toString().trim()))
		{
	serachAddress+=""+arr_formatted_address[i].toString().trim().replace(/ /g, '+')+"+";
}else if(i<2)
...
var formatted_address=arr_formatted_address[z-1].replace('+','');
var formatted_address2=arr_formatted_address[0].replace('+','');//.long_name;
         $.ajax({
           type: "Get",
           url: 'https://nominatim.openstreetmap.org/search.php?q='+serachAddress+'&polygon_geojson=1&format=json',
           data: {},
...
var tLongitude="";
var tLatitude="";
           	for (var i = 0; i <data[j].geojson.coordinates[0].length ; i++) {
           		if (data[j].display_name.toString().toLowerCase().includes(formatted_address.toString().toLowerCase())&&isValid&&data[j].geojson.type.toString().toLowerCase()=='polygon'&&data[j].display_name.toString().toLowerCase().includes(formatted_address2.toString().toLowerCase())) {
           				 			triangleCoords.push(new google.maps.LatLng(data[j].geojson.coordinates[0][i][1],data[j].geojson.coordinates[0][i][0]));
           				 		var point = new google.maps.LatLng(data[j].geojson.coordinates[0][i][1],data[j].geojson.coordinates[0][i][0]);
           				 		bounds.extend(point);
           				 		if (data[j].geojson.coordinates[0][i][0]) {
tLatitude+=data[j].geojson.coordinates[0][i][1]+",";
tLongitude+=data[j].geojson.coordinates[0][i][0]+",";
}
}

	
           	}
           	if (tLatitude.trim()!==""&&tLongitude.trim()!=="") {
$('#tLatitude').val(tLatitude);
		$('#tLongitude').val(tLongitude);	
	isValid=false;
			// Styling & Controls
