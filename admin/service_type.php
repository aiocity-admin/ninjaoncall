...
if ($sortby==7) {
   ...
   if($order == 0)
  $ord = " ORDER BY  vcParent.vCategory_".$default_lang." ASC";
  else
  $ord = " ORDER BY  vcParent.vCategory_".$default_lang." DESC";
...
if($keyword != ''){
    if($option != '') {
      ...
      if($iVehicleCategoryId != '') {
        $ssql.= " AND (vt.vVehicleType_".$default_lang." LIKE '%".$keyword."%' OR vt.fPricePerKM LIKE '%".$keyword."%' OR vt.fPricePerMin LIKE '%".$keyword."%' OR vt.iPersonSize	 LIKE '%".$keyword."%' or vcParent.vCategory_".$default_lang." LIKE '%".$keyword."%' or vc.vCategory_".$default_lang." LIKE '%".$keyword."%') AND vt.iVehicleCategoryId = '".$iVehicleCategoryId."'";
      } else {
        $ssql.= " AND (vt.vVehicleType_".$default_lang." LIKE '%".$keyword."%' OR vt.fPricePerKM LIKE '%".$keyword."%' OR vt.fPricePerMin LIKE '%".$keyword."%' OR vt.iPersonSize   LIKE '%".$keyword."%' or vcParent.vCategory_".$default_lang." LIKE '%".$keyword."%' or vc.vCategory_".$default_lang." LIKE '%".$keyword."%')";
      }
	}
 ...
 else
{
	...
	if($Vehicle_type_name == 'UberX')
	{
/*		$sql = "SELECT vt.*,vc.iVehicleCategoryId,vc.vCategory_".$default_lang.",lm.vLocationName from  vehicle_type as vt  
    ...
    left join location_master as lm ON lm.iLocationId = vt.iLocationid 
		where vt.eType='".$Vehicle_type_name."' $ssql $adm_ssql $ord LIMIT $start, $per_page"; codeEdited for adding category column	*/
     $sql = "SELECT vt.*,vc.iVehicleCategoryId,vc.vCategory_".$default_lang.",lm.vLocationName,vcParent.vCategory_".$default_lang." as category
    from  vehicle_type as vt  
    left join vehicle_category as vc on vt.iVehicleCategoryId = vc.iVehicleCategoryId 
    left join country as c ON c.iCountryId = vt.iCountryId
    left join state as st ON st.iStateId = vt.iStateId
    left join city as ct ON ct.iCityId = vt.iCityId
    left join location_master as lm ON lm.iLocationId = vt.iLocationid left join vehicle_category vcParent on  vc.iParentId=vcParent.iVehicleCategoryId
...
<!--                                     <input type="hidden" name="option" id="option" value="vVehicleType_<?=$default_lang?>">
 -->                                    <?php } ?>
                                    ...
                                    <td width="30%"><a class="add-btn" href="service_type_action.php" style="text-align: center;">Add <?php echo $langage_lbl_admin['LBL_SERVICE_TXT'];?> Type</a></td>
                                </tr>
                              </tbody>
                        </table>
                                                ...
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th width="15%"><a href="javascript:void(0);" onClick="Redirect(1,<?php if($sortby == '1'){ echo $order; }else { ?>0<?php } ?>)">Type<?php if ($sortby == 1) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?>  <i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
                            														 ...
                            														 <?php if($app_type_service == 'UberX'){ ?> 
   <th width="10%">
<a href="javascript:void(0);" onClick="Redirect(7,<?php if($sortby == '7'){ echo $order; }else { ?>0<?php } ?>)">Service category<?php if ($sortby == 7) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?>  <i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a>
                                                         </th>

                            														 <th width="10%">
<a href="javascript:void(0);" onClick="Redirect(6,<?php if($sortby == '6'){ echo $order; }else { ?>0<?php } ?>)">Sub category<?php if ($sortby == 6) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?>  <i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a>
                                                         </th>
                                                          <th width="8%" align="center" style="text-align:center;"><a href="javascript:void(0);" onClick="Redirect(5,<?php if($sortby == '5'){ echo $order; }else { ?>0<?php } ?>)"> Display Order <?php if ($sortby == 5) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
                            														 <?php } ?>
                    ...
                    <div class="admin-notes">
                      ...
                      <ul>
                        <li>
                          <?php echo $langage_lbl_admin['LBL_SERVICE_TXT'];?> Type module will list all <?php echo $langage_lbl_admin['LBL_SERVICE_TXT'];?> types on this page.
                        </li>

                        <li>
                          Administrator can Edit / Delete any <?php echo $langage_lbl_admin['LBL_SERVICE_TXT'];?> type. 
                        </li>
