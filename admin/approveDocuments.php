...
$status = isset($_REQUEST['status'])?stripslashes($_REQUEST['status']):"";

$ssql = '';

if(isset($_POST['ChangeStatus']))
{
$statusUpdate = isset($_REQUEST['statusUpdate'])?stripslashes($_REQUEST['statusUpdate']):"";
...
if($keyword != ''){
    ...
    if($option != '') {
     ...
     $ssql.= " AND dl.IsApprove = '".$generalobjAdmin->clean($status)."'";
}
    } else {
        if($status != ''){
            $ssql.= " AND (c.vEmail LIKE '%".$keyword_s."%' OR rd.vEmail LIKE '%".$keyword_s."%' OR  concat(rd.Suffix,rd.vName,rd.MiddleName,rd.vLastName) LIKE '%".$keyword_s."%' OR  c.vCompany LIKE '%".$keyword_s."%' OR dm.doc_usertype LIKE '%".$keyword_s."%' OR dm.doc_name LIKE '%".$keyword."%')";
              if($status != '' && $status!='Expired') 
     $ssql.= " AND dl.IsApprove = '".$status."'";
        } else {
            $ssql.= " AND (c.vEmail LIKE '%".$keyword_s."%' OR rd.vEmail LIKE '%".$keyword_s."%' OR concat(rd.Suffix,rd.vName,rd.MiddleName,rd.vLastName) LIKE '%".$keyword_s."%' OR c.vCompany LIKE '%".$keyword_s."%' OR dm.doc_usertype LIKE '%".$keyword_s."%' OR dm.doc_name LIKE '%".$keyword_s."%')";
        }
    }
...
$sql = "SELECT COUNT(dl.doc_id) AS Total FROM `document_list` dl left join document_master dm on dm.doc_masterid=dl.doc_masterid  LEFT join register_driver rd ON rd.iDriverId=dl.doc_userid and dl.doc_usertype!='company' and rd.eStatus!='Deleted'  LEFT join company c on c.iCompanyId=dl.doc_userid and dl.doc_usertype='company'  and c.eStatus!='Deleted'   WHERE 1=1 and dl.status!='Deleted' $eStatussql $ssql";
...
$sql = "SELECT dl.doc_id,dl.doc_masterid, dl.doc_usertype , dl.doc_file, dl.IsApprove ,dl.ex_date,dl.IsApprove,dm.doc_name,dm.eType, case when concat(rd.Suffix,'',rd.vName,'',rd.MiddleName,'',rd.vLastName) is null then c.vCompany else concat(rd.Suffix,' ',rd.vName,' ',rd.MiddleName,' ',rd.vLastName) end as Name, case when dl.ex_date < '".date("Y-m-d")."' and ex_status='yes' then 'true' else 'false' end as IsExpired,dl.edate,dl.doc_userid,case when rd.vEmail is null then c.vEmail else rd.vEmail end as vEmail  FROM `document_list` dl left join document_master dm on dm.doc_masterid=dl.doc_masterid  LEFT join register_driver rd ON rd.iDriverId=dl.doc_userid and dl.doc_usertype!='company' and rd.eStatus!='Deleted' LEFT join company c on c.iCompanyId=dl.doc_userid and dl.doc_usertype='company'  and c.eStatus!='Deleted'   WHERE 1=1 and dl.status!='Deleted' $eStatussql $ssql $ord LIMIT $start, $per_page"; 
//echo $sql;
                ...
                <div class="col-md-1">
                 ...
                 <div class="col-md-2">
                    <select name="option" id="option" class="form-control">
                        ...
                        <option value="company" <? if($option=='company') echo "selected"; ?>>Company</option>
                                                        ...
                                                        <td><?= $data_drv[$i]['doc_name']; ?></td>
                                                            ...
                                                            <?php  if($data_drv[$i]['IsExpired'] == 'true'){
                                                                ?>
                                                                 ...
                                                                 <img src="img/expired-icon.png" alt="<?=$data_drv[$i]['IsExpired'];?>" data-toggle="tooltip" title="Expired">
                                                                 <?php                                                                
                                                            } ?>
                                                                    ...
                                                                    <ul style="height:178px;">
                                                                      <!--   <li class="entypo-twitter" data-network="twitter"><a href="document_master_add.php?id=<?= $data_drv[$i]['doc_masterid']; ?>" data-toggle="tooltip" title="Edit">
                                                                        ...
                                                                        <li class="entypo-gplus" data-network="gplus"><a href="javascript:void(0);" onclick="requestToResubmit('<?php echo $data_drv[$i]['doc_userid']; ?>','<?php echo $data_drv[$i]['doc_usertype'];?>','<?php echo $data_drv[$i]['doc_name']; ?>','<?php echo $url;?>')"   data-toggle="tooltip" title="Resubmit">
                                                                            <img src="img/resubmit-icon.png" alt="Resubmit" >
                                                                        </a></li>
    ...
    ?>
     <script>
      ...
      data={iCompanyId:id,document:docName,url:url,status:status};
            ...
            $.ajax({
  url: "documentResubmitRequest.php",
  type: "POST",
  data: data,
  cache: false,
  async:isAsync,
