...
$sql_admin = "SELECT tr.iTripId,tr.vRideNo,tr.iDriverId,tr.iUserId,tr.tTripRequestDate, tr.eType, tr.eHailTrip,tr.fTripGenerateFare,tr.fCommision, tr.fDiscount, tr.fWalletDebit, tr.fTipPrice,tr.eDriverPaymentStatus,tr.ePaymentCollect,tr.vTripPaymentMode,tr.iActive,tr.fOutStandingAmount, tr.iRentalPackageId,c.vCompany,concat(rd.vName,' ',rd.vLastName) as drivername,concat(ru.vName,' ',ru.vLastName) as riderName,tr.ServiceFeeType,tr.RevenueShare,tr.FixedCommision,case when vcp.vCategory_EN is NULL or vcp.vCategory_EN='' then tr.eType else  vcp.vCategory_EN end as vCategory_EN,vt.vVehicleType FROM trips AS tr LEFT JOIN register_driver AS rd ON tr.iDriverId = rd.iDriverId LEFT JOIN vehicle_type vt ON vt.iVehicleTypeId = tr.iVehicleTypeId left join vehicle_category vc on vc.iVehicleCategoryId=vt.iVehicleCategoryId left join (SELECT * FROM  vehicle_category  WHERE  `iParentId`=0) as vcp on vcp.iVehicleCategoryId=vc.iParentId  LEFT JOIN register_user AS ru ON tr.iUserId = ru.iUserId LEFT JOIN company as c ON rd.iCompanyId = c.iCompanyId WHERE  if(tr.iActive ='Canceled',if(tr.vTripPaymentMode='Card',1=1,0),1=1) AND tr.iActive ='Finished' $ssql $trp_ssql $ord ";
    ...
    $header .= "E=Trip Outstanding Amount" . "\t";
...
$ServiceFeeType="";
switch ($db_trip[$j]['ServiceFeeType']) {
    case '1':
       
       $ServiceFeeType= 'Revenue Share Only';
   ...
   /* case '3':
       echo 'Token Fee Only';
        break;*/
           case '4':
        # code...
            $ServiceFeeType= 'Revenue Share and Fixed Fee';
        break;
          /* case '5':
        # code...
        ...
        break;   */    
   case '6':
            $ServiceFeeType= 'Quantity based Fixed Fee';
        break;      
}
     ...
     $data .= $ServiceFeeType . "\t";
      ...
      $data .= ($db_trip[$j]['FixedCommision'] != "" && $db_trip[$j]['FixedCommision'] != 0) ? $db_trip[$j]['FixedCommision'] . "\t" : "- \t";
     $data .= ($db_trip[$j]['RevenueShare'] != "" && $db_trip[$j]['RevenueShare'] != 0) ? $db_trip[$j]['RevenueShare'] . "\t" : "- \t";
        ...
        $data .=$db_trip[$i]['fOutStandingAmount']. "\t";


        $data .= ($driver_payment != "" && $driver_payment != 0) ? $driver_payment . "\t" : "- \t";
        $data .= $db_trip[$j]['iActive'] . "\t";
        $data .= $paymentmode . "\t";
        $data .= ($db_trip[$j]['eDriverPaymentStatus']=="Settelled"?"Settled":$db_trip[$j]['eDriverPaymentStatus'] ). "\n";
    }
    $data .= "\n\t\t\t\t\t\t\t\t\tTotal Fare\t" . $tot_fare. "\n";
...
for ($j = 0; $j < count($db_dlip); $j++) {
  ...
  //       }
                       ...
                       //$data .= ($db_dlip[$j]['vCategory_EN']=='Deliver'?'Delivery':$db_dlip[$j]['vCategory_EN']). "\t";
		$data .=  $trip_type. "\t";
...
if($eType!="")
{
  ...
  if ($eType=='Ride' || $eType=="Deliver")  
{

    $ssql .= " AND   crn.eType = '".$eType."' ";

}else{
    $ssql .= " AND   crn.iVehicleTypeId = '".$eType."' ";
  }
$join="left join cab_request_now crn on rs.iRequestId=crn.iCabRequestId";
}

        $chk_str_date = @date('Y-m-d H:i:s', strtotime('-'.$RIDER_REQUEST_ACCEPT_TIME.' second'));
        $sql_admin = "SELECT rd.iDriverId , rd.vLastName ,rd.vName ,c.vCompany,
        ...
        FROM register_driver rd  left join driver_request rs on rd.iDriverId=rs.iDriverId left join company c on rd.iCompanyId=c.iCompanyId $join   WHERE 1=1 $ssql GROUP by rs.iDriverId $ord ";
/*				
		...
		$db_dlip = $obj->MySQLSelect($sql_admin);
			...
			for ($j = 0; $j < count($db_dlip); $j++) {
					...
					$data .= $generalobjAdmin->clearName($db_dlip[$j]['vCompany']). "\t";
		...
		if($serachTripNo!=''){
			$ssql.=" AND t.vRideNo ='".$serachTripNo."'";
			 ...
			 for ($j = 0; $j < count($db_dlip); $j++) {
					 ...
					 $data .= $db_dlip[$j]['vRideNo']. "\t";
					 $data .= str_replace(array("\n", "\r", "\r\n", "\n\r"),' ',$db_dlip[$j]['tSaddress'].' -> '.$db_dlip[$j]['tDaddress'] ) . "\t";
...
/*			$ord = ' ORDER BY iUserWalletId DESC';*/
            $ord = ' ORDER BY dDate ASC';
				...
				$header .= "Description" . "\t";
				$header .= "Amount". "\t";
				$header .= "Booking No/Reference no" . "\t";
				...
				$header.=   "Service Fee". "\t";						
					...
					$data .= $db_dlip[$j]['iBalance'] . "\t";
					$data .= $ride_number ."\t";
					$data .= $generalobjAdmin->DateTime($db_dlip[$j]['dDate']) ."\t";
					$data .= $db_dlip[$j]['eFor']."\t";
					$data .= $db_dlip[$j]['eType']."\t";
 					$data .=$db_dlip[$j]['currentbal']."\t";
                     $data .=$db_dlip[$j]['ServiceFee']."\n";
