...
$debit_query="INSERT INTO `barangay_tokens`(`BarangayId`, `Tokens`, `Type`, `TDate`, `DriverId`) VALUES ('$BarangayId','$iBalance','Debit','$dDate','$iUserId')";
 ...
 $_POST['action']="";
     $_SESSION['success'] = '1';
    $_SESSION['var_msg'] = $langage_lbl_admin["LBL_DRIVER_TXT_ADMIN"].' in add tokens successfully';   
  header("Location:".$tconfig["tsite_url_main_admin"]."barangay_providers.php"); exit;
   //exit;
  }
  else{
      $_SESSION['error'] = '1';
  $not_have_enough_tokens='You do not have enough tokens';   
...
$admin_id=$_SESSION['sess_iAdminUserId'];
  ...
  $db_company= $obj->MySQLSelect($sql);
     ...
     $record_for_CompanyId =  $db_company[0]['Company'];


$sql = "SELECT COUNT(iDriverId) AS Total FROM register_driver rd LEFT JOIN company c ON rd.iCompanyId = c.iCompanyId WHERE 1 = 1  $eStatussql $ssql $ssql1 $dri_ssql and c.iCompanyId = '". $record_for_CompanyId."'";
                              ...
                              <tbody>
                                <tr>
                                    ...
                                    <td>
                                      ...
                                      <input type="button" value="Reset" class="btnalt button11" onClick="window.location.href='barangay_providers.php'"/>
                                    </td>
                                    <td width="30%"><a class="add-btn" href="driver_action.php" style="text-align: center;">Add <?=$langage_lbl_admin['LBL_DRIVER_TXT_ADMIN'];?></a></td>
                                                            ...
                                                            <?php
if (trim($_SESSION['error'])=="1") {
  # code...

                                                             echo "<div class='alert alert-danger'> <strong>Failure!</strong> ".$not_have_enough_tokens.". <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a></div>";
$_SESSION['error']='';
}
                                                              ?>
                        ...
                        <div class="row">
                          ...
                          if(!empty($data_drv)) {
                                ...
                                if($data_drv[$i]['count'] != 0) {?>
                                                                            <td align="center"><a href="barangay_providers.php?&actionSearch=1&iDriverId=<?=$data_drv[$i]['iDriverId'];?>" target="_blank">--><?php //echo $data_drv[$i]['count'];?><!--</a></td>  
                                                                        <?php } else { ?>
                                  ...
                                  <?php } ?>
                                                                           ...
                                                                           if($data_drv[$i]['eStatus'] != "Deleted"){ 
                                                                            ...
                                                                            echo $user_available_balance;
                                                                                ?>
                                                                           <button type="button" onClick="Add_money_driver('<?=$data_drv[$i]['iDriverId'];?>')" class="btn btn-success btn-xs">Add Tokens</button>
                                                                           <?php }else{
                                                                                echo $generalobj->trip_currency($user_available_balance);
                                                                                } ?>
            ...
            <!--END MAIN WRAPPER -->
<form name="pageForm" id="pageForm" action="action/barangay_providers.php" method="post" >
