...
foreach ($extraField as $key => $value) {
    $subQuery.=",ExtraField_".$z++."='".$value."'";
	        ...
	        $query="UPDATE `".$tbl."` SET `doc_file`='".$vImage."' , `ex_date`='".$expDate."',`DocumentTypeName`='".$documentType."' $subQuery WHERE doc_userid='".$_REQUEST[id]."' and doc_usertype='car'  and doc_masterid=".$_REQUEST['doc_type'];
/*                $q = "UPDATE ";
    ...
    $subQuery_col.=",ExtraField_".$z++;
      $subQuery_val.=",'".$value."'";
            ...
            $query =" INSERT INTO `".$tbl."` ( `doc_masterid`, `doc_usertype`, `doc_userid`, `ex_date`, `doc_file`, `status`, `edate`,`DocumentTypeName` $subQuery_col) "
                   . "VALUES "
                   . "( '".$_REQUEST['doc_type']."', 'car', '".$_REQUEST['id']."', '".$expDate."', '".$vImage."', 'Inactive', CURRENT_TIMESTAMP,'".$documentType."' $subQuery_val)";
...
foreach ($extraField as $key => $value) {
    $subQuery_col.=",ExtraField_".$z++;
      $subQuery_val.=",'".$value."'";
        ...
        $query =" INSERT INTO `".$tbl."` ( `doc_masterid`, `doc_usertype`, `doc_userid`, `ex_date`, `doc_file`, `status`, `edate`,`DocumentTypeName` $subQuery_col) "
               . "VALUES " . "( '".$_REQUEST['doc_type']."', 'car', '".$_REQUEST['id']."', '".$expDate."', '".$vImage."', 'Inactive', CURRENT_TIMESTAMP,'".$documentType."' $subQuery_val)";
        }
        $obj->sql_query($query);
        //Start :: Log Data Save
        if (empty($check_file[0]['vNoc'])) {
